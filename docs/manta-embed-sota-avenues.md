...
MS MARCO hard-candidate/dual-teacher status: research-only lineage, with `release_train_allowed=false`, `commercial_use_allowed=false`, and `train_allowed_for_research=true`. The hard-candidate substrate has `5,000` rows and `135,565` negatives from bounded BM25, lexical-overlap, and random controls; the current-Eos full-corpus lane is blocked by no local MS MARCO cache. Qwen3 hard scoring reached positive top1 `0.6108`; mxbai hard scoring reached positive top1 `0.6626`. The clean agreement split has `2,379` rows with no global dev-positive negative refs; `teacher_scores` are averaged per-row z-score Qwen3/mxbai vectors, and audit reports positive_top1 `1.0`, mean margin `2.191931`, and entropy `0.599264`. The conflict/adjudication queue has strict `2,200` rows, including `5` zero-margin both-top1 rows, and `8,800` reranker request rows. A bounded training smoke ran one epoch from the clean artifact with LR `2.5e-8`, `grouped_infonce`, HN3, `teacher_loss_weight=0.05`, and pairwise-only quality target; training and package verify succeeded, but no full retrieval scoreboard was run, final/hard top1 stayed unchanged, and AUC/margin moved slightly down. Treat the clean distillation artifact as mechanically valid research data only; do not promote, full-scoreboard, or continue this exact setting as a quality win. If continuing, change one knob at a time, starting with LR or teacher/loss weighting, require matched pairwise/hard lift before any retrieval scoreboard, and adjudicate the conflict queue separately.
