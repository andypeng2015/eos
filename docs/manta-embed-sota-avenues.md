...
Current capped LongEmbed evidence for the same s40 line is diagnostic and not a product-quality claim. The `eos-long-context-wedge-matrix-v1` aggregation preserves `quality_claim=false` and shows current Eos does not beat strong chunked baselines on the key comparable real-doc rows: over `qmsum` + `2wikimqa`, best Eos macro nDCG@10 is `0.558927` versus Qwen3 q4 `0.918122` and mxbai q4 `0.864007`, for an Eos-best gap of `-0.359194`; the biggest row gap is `2wikimqa` at `-0.384542`. The synthetic `passkey4096` fusion win is `+0.127878`, but this is diagnostic fullslice evidence, not product-quality proof. Preserve `quality_claim=false`, capped-slice caveats, synthetic-task caveats, and external child-count/storage caveats when using these rows. The next long-context claim needs a changed objective/model or more comparable real-doc external rows before any product-quality claim.
