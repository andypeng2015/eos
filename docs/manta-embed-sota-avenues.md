...
Jina v2 HF fallback and BGE-large TEI are now both negative full-frontier model-quality evidence. On the cleaned full-frontier rebuild at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/`, Jina collapsed to `160/1188 = 0.134680` selected/any-positive top1. On the same cleaned/provenance-safe frontier, `BAAI/bge-reranker-large` scored `7128/7128` request rows in `1:03.02` with `duplicates_skipped=0`, imported `1188` examples with missing `0`, audited `1188` scored / `0` missing / `0` duplicate-positive negatives, and selected only `142/1188 = 0.119529` positives at top-1. Mean normalized entropy was `0.993488`; the filter kept `142` and cleared `1046`. BGE artifacts are under `debug-bge-large-full-clean-v1/` and `data/nfcorpus.train-dev.cleaned-full-frontier.bge-large.*`; the existing TEI cache was reused, no download was needed, the container was cleaned up, and final worker status was clean. `mixedbread-ai/mxbai-rerank-base-v2` remains TEI-blocked, but HF/SentenceTransformers CrossEncoder scoring was mechanically unblocked in `runs/eos-mxbai-rerank-base-hf-unblock-smoke-v1-20260623T124344Z`. Its tiny raw-logit smoke passed; the cleaned-frontier subset64 then scored/imported/audited/filtered at positive top1 `1/64 = 0.015625`, mean margin `-7.836426`, mean normalized entropy `0.476268`, filter kept `1`, and cleared `63`. The subset failed the `0.35` gate, so full scoring was skipped and there is no full-frontier mxbai-base metric. Mark Jina, BGE-large, and this mxbai-base subset `teacher_quality_blocked` / `no_train`, with mxbai-base specifically `subset_quality_gate_failed`; do not train from these rows. `mixedbread-ai/mxbai-rerank-large-v2` remains not evaluated for quality. `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` was not rerun on cleaned data; its historical dirty-subset audit stayed no-train at positive-top1 `0.078125`, kept `5/64`, mean margin `-0.5452850256115198`. The next descriptor should pivot to capacity/bootstrap, a compact/Matryoshka objective, or a different data strategy.
