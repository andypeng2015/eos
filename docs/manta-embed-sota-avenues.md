...
The nf005 package is the dense release-candidate line. Its promoted compact policy is q4/fp16/rerank-overfetch=200, method `turboquant_ip_b4_overfetch200_fp16_rerank`, total compression `1.5900621118x`. It is selected because direct checked-in q4/fp16/175 failed FiQA recall versus predecessor q4/fp16/250, while q4/200 is the lowest historical recovery-sweep profile that clears predecessor quality across SciFact, NFCorpus, and FiQA by explicit comparison: SciFact nDCG delta `+0.002216`, recall delta `0`; NFCorpus nDCG delta `+0.001402`, recall delta `+0.000168`; FiQA nDCG delta `+0.000815`, recall delta `+0.001337`. That legacy recovery sweep is historical selection evidence and the stricter FiQA recall reference (`0.3516782086`), but it lacks `quantizer_seed` and is not the formal compact `gate-scoreboard` anchor. The formal seeded q4/fp16/o200 compact anchor is `runs/eos-nf005-compact-anchor-provenance-repair-v1-20260619T091223Z/anchor-q4-fp16-overfetch200-scoreboard/scoreboard.json`; its selected rows carry `quantizer_seed=5581486560434873699` and record FiQA recall `0.3512924062` and NFCorpus recall `0.2436904780`. Future compact-aware model candidates should report both the formal seeded gate and the historical strict FiQA reference until a new release policy supersedes it. The compact-aware prepared-IP q4 rerun is no-promote because it matches the seeded compact behavior but does not improve over nf005 and does not clear the historical stricter FiQA reference. q4/150 was not selected because later paired evidence showed recall instability. Separate current nf005 CorkScrewDB local flat packed-parent API evidence now exists in `runs/eos-nf005-current-default-corkscrewdb-budget-quality-packed-q4q8-20260617T142823Z/`: q4 `packed_parent_multivectors` with `metadata=none` and ordinal child IDs recorded nDCG@10 `0.451122`, recall@100 `0.755222`, DB directory multiple `0.020372x`, vector payload multiple `0.013312x`, planner fit `180`, target fit `true`, and p95 `142.971679ms`. q8 is diagnostic only: nDCG@10 `0.472867`, recall@100 `0.780222`, DB directory multiple `0.032901x`, p95 `22.988553ms`, planner fit `93`, and target fit `false`. This API evidence is not proof of remote mode, federation, HNSW, q4/fp16 alias promotion, or hosted-model parity, and the q4 p95 is run evidence rather than a service SLO.
