...
Dense-quality next lane status: `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` generated a request-only NFCorpus train/dev frontier for stronger reranker teaching, with `1,188` hard-negative examples and `7,066` scorer request rows. The true-reranker scorer path is now mechanically unblocked: `cmd/teacher-bridge --mode tei-rerank` works against TEI `/rerank`, and RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9` rather than the generic CUDA image. Decision remains `no_train` / `teacher_quality_blocked`: cleaned subset64 BGE reranker audits are still quality-negative, embedding/cosine or vector-cache teachers are not a substitute for this lane, and generated rows remain `request_only=true` plus `train_allowed=false`. Cleaning removed the duplicate-positive-negative failure mode from the audited BGE slice (`duplicate_positive_negative_candidates=0` for each cleaned BGE run), but the best cleaned BGE teacher still kept only `29/64`. Qwen3 seq-cls was tested only on the earlier dirty subset as another no-train teacher: TEI `120-1.9` failed with `classifier model type is not supported for Qwen3`, while Python `AutoModelForSequenceClassification` with Qwen3 model-card formatting on CUDA RTX 5070 Ti was mechanically scoreable but quality-negative. Do not cite Qwen3 as cleaned-data evidence. Investigate label/teacher disagreement or test a different query-document teacher before full scoring or proposing dense training.
...
BGE dirty-to-cleaned selected-top1 changes were base `+2`, large `+7`, and v2-m3 `+1`; the cleaned BGE audits report `duplicate_positive_negative_candidates=0` and equal selected/any-positive top1 because no exact-text duplicate negative remained qrels-positive. Qwen3 seq-cls mean margin on the earlier dirty subset was `-0.5452850256115198`; filtering kept `5/64`, cleared `59`, dropped `0`, and preserved request-only/no-train metadata. Metadata preservation through import/filter was verified on subset64 after `a7482cc`; the evidence remains audit-only/no-train until a teacher passes positive-top1, margin, novelty, and leakage gates.
...
After scoring it with a real reranker service, import the scored rows with `eos import-teacher-scores`, then run `audit-teacher-scores` and `filter-teacher-scores`. When qrels and corpus provenance exist, run `audit-teacher-scores --qrels <qrels.tsv> --corpus <corpus.jsonl> <scored-hard-negatives.jsonl> <summary.json>` to add duplicate/multi-positive-aware diagnostics while preserving the selected-positive defaults. Training remains blocked until real scorer output passes positive-top1, margin, novelty, and leakage gates; fake scorer or embedding-cosine bridge output is only plumbing evidence. Current cleaned subset64 BGE true-reranker audits remain no-train: `BAAI/bge-reranker-base` selected/any-positive top1 `29/64` (`0.453125`), kept `29/64`; `BAAI/bge-reranker-large` `22/64` (`0.343750`), kept `22/64`; and `BAAI/bge-reranker-v2-m3` `10/64` (`0.156250`), kept `10/64`. All three cleaned BGE audits report `duplicate_positive_negative_candidates=0`; dirty-to-cleaned selected-top1 changed by base `+2`, large `+7`, and v2-m3 `+1`. `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` was not rerun on cleaned data; its historical dirty-subset audit stayed no-train at positive-top1 `0.078125`, kept `5/64`, mean margin `-0.5452850256115198`. Qwen3 seq-cls TEI `120-1.9` startup failed because `classifier model type is not supported for Qwen3`; the Python `AutoModelForSequenceClassification` fallback on CUDA RTX 5070 Ti was mechanically scoreable with Qwen3 model-card formatting, but it also failed the quality gate.
