...
The cleaned, provenance-safe full frontier at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/` closes both Jina and BGE-large as reranker-teacher lanes for this frontier. Jina v2 scored `7128/7128` request rows and selected the positive at top-1 for only `160/1188 = 0.134680`. `BAAI/bge-reranker-large` then reused the same cleaned frontier, existing TEI cache, and no-download setup; its debug/artifacts are under `debug-bge-large-full-clean-v1/` and `data/nfcorpus.train-dev.cleaned-full-frontier.bge-large.*`. BGE-large scored `7128/7128` request rows in `1:03.02` with `duplicates_skipped=0`, imported `1188` examples with missing `0`, audited `1188` scored / `0` missing / `0` duplicate-positive negatives, and selected only `142/1188 = 0.119529` positives at top-1. Mean normalized entropy was `0.993488`; the filter kept `142` and cleared `1046`. Mark BGE-large, like Jina, `teacher_quality_blocked` / `no_train`; no guarded trainability descriptor should be proposed from this cleaned full frontier. The TEI container was cleaned up and final worker status was clean. `mixedbread-ai/mxbai-rerank-base-v2` and `mixedbread-ai/mxbai-rerank-large-v2` still failed TEI `120-1.9` serving even with `--pooling cls` / `--pooling mean` because `sentence_bert_config.json.max_seq_length` is missing; treat those as compatibility blockers, not model-quality evidence. Generated rows remain `request_only=true` plus `train_allowed=false`; do not train or promote from these artifacts. The next dense-quality action should pivot to mxbai compatibility if feasible, capacity/bootstrap, a compact/Matryoshka objective, or a different data strategy.
