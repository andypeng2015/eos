...
Metadata preservation through import/filter was verified on subset64 after `a7482cc`; the evidence remains audit-only/no-train until a teacher passes positive-top1, margin, novelty, and leakage gates.
...
After scoring it with a real reranker service, import the scored rows with `eos import-teacher-scores`, then run `audit-teacher-scores` and `filter-teacher-scores`. Training remains blocked until real scorer output passes positive-top1, margin, novelty, and leakage gates; fake scorer or embedding-cosine bridge output is only plumbing evidence. Current BGE subset64 true-reranker audits are no-train: `bge-reranker-base` positive-top1 `0.421875` kept `27/64`, `BAAI/bge-reranker-v2-m3` positive-top1 `0.140625` kept `9/64`, and `BAAI/bge-reranker-large` positive-top1 `0.234375` kept `15/64`.
