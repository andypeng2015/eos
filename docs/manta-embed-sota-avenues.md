...
Dense-quality next lane status: `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` generated a request-only NFCorpus train/dev frontier for stronger reranker teaching, with `1,188` hard-negative examples and `7,066` scorer request rows. The true-reranker scorer path is now mechanically unblocked: `cmd/teacher-bridge --mode tei-rerank` works against TEI `/rerank`, and RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9` rather than the generic CUDA image. Decision remains `no_train` / `teacher_quality_blocked`: tested subset64 teachers failed positive-top1/margin quality gates, embedding/cosine or vector-cache teachers are not a substitute for this lane, and generated rows remain `request_only=true` plus `train_allowed=false`. Qwen3 seq-cls was tested as another no-train teacher: TEI `120-1.9` failed with `classifier model type is not supported for Qwen3`, while Python `AutoModelForSequenceClassification` with Qwen3 model-card formatting on CUDA RTX 5070 Ti was mechanically scoreable but quality-negative. Investigate label/teacher disagreement or test a different query-document teacher before full scoring or proposing dense training.

| subset64 teacher | runtime | positive_top1 | kept |
| --- | --- | ---: | ---: |
| `BAAI/bge-reranker-base` | TEI `cpu-1.9` | `0.421875` | `27/64` |
| `BAAI/bge-reranker-v2-m3` | TEI `120-1.9` | `0.140625` | `9/64` |
| `BAAI/bge-reranker-large` | TEI `120-1.9` | `0.234375` | `15/64` |
| `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` | Python seq-cls fallback; TEI `120-1.9` unsupported for Qwen3 classifier | `0.078125` | `5/64` |

Qwen3 seq-cls mean margin was `-0.5452850256115198`; filtering kept `5/64`, cleared `59`, dropped `0`, and preserved request-only/no-train metadata. Metadata preservation through import/filter was verified on subset64 after `a7482cc`; the evidence remains audit-only/no-train until a teacher passes positive-top1, margin, novelty, and leakage gates.
...
After scoring it with a real reranker service, import the scored rows with `eos import-teacher-scores`, then run `audit-teacher-scores` and `filter-teacher-scores`. Training remains blocked until real scorer output passes positive-top1, margin, novelty, and leakage gates; fake scorer or embedding-cosine bridge output is only plumbing evidence. Current subset64 true-reranker audits are no-train: `bge-reranker-base` positive-top1 `0.421875` kept `27/64`, `BAAI/bge-reranker-v2-m3` positive-top1 `0.140625` kept `9/64`, `BAAI/bge-reranker-large` positive-top1 `0.234375` kept `15/64`, and `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` positive-top1 `0.078125` kept `5/64` with mean margin `-0.5452850256115198`. Qwen3 seq-cls TEI `120-1.9` startup failed because `classifier model type is not supported for Qwen3`; the Python `AutoModelForSequenceClassification` fallback on CUDA RTX 5070 Ti was mechanically scoreable with Qwen3 model-card formatting, but it also failed the quality gate.
