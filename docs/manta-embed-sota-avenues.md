...
Status: s40 NFCorpus compact-mined narrow release package, sealed and dense short-set gate verified against the previous s40 default. The release sealed artifact SHA256 is `e0eca16ff34ebb88ca96862d58c3ac7f02dbf4b124599fdf96f25344ac02e408`; release package SHA256 is `188265db16992ab24be15e678c5f7e175bebad769e8d844e8b0f50ffc23bd5bf`; tokenizer SHA256 is `64cf63223cb3f97125040677a573e6ab6c625cff1f6f338f4e680a4c9f7a42f5`; training JSONL SHA256 is `72830906ac4e9e57a6c108002187f059654a818ca61265ef2f14a445d88418b0`. Package and sealed inspection report `package verify: OK`. The training data was `train-anchor-plus-nfcorpus-compact-mined-64.jsonl` with 102 protected anchor rows plus 64 NFCorpus train-mined compact hard-negative rows, no NFCorpus test rows used for training or selection, LR `0.000000025`, HN `5`, compact objective `256:4=0.05`, prefix seed `5581486560434873699`, and dev-mined hard eval only. The predecessor s40 package, nf005 package, targeted-v3 package, and legacy source artifact remain comparison provenance only.
...
Dense comparison against the previous s40 default:
...
s40-era hybrid policy evidence exists for the previous durable default asset SHA256 `f494915a0d78b24205d5018bb701bf40cabbedee4bc8b96b6a1920b19131da5a`, under `runs/eos-s40-current-default-hybrid-policy-v1-20260621T103500Z/`. The command-level path uses the same `eval-retrieval-hybrid --method minmax_blend --alpha 0.5 --top-k 100` policy over the s40 dense anchor and passes all three datasets:
...
This is local lexical+dense ranking-policy evidence for the previous s40 asset, not dense model promotion. Refresh it before claiming hybrid/API coverage for the new default.

The narrow package is the dense and compact release-candidate line. Its promoted compact policy is q4/fp16/rerank-overfetch=200, method `turboquant_ip_b4_overfetch200_fp16_rerank`, total compression `1.5900621118x`. It passed strict seeded compact non-regression against the s40 q4/fp16/o200 anchor: SciFact unchanged; NFCorpus nDCG@10 `+0.000174802593872`, recall@100 `+0.000006604583581`; FiQA nDCG@10 `+0.000180234154897`, recall@100 `+0.000771604938272`; macro nDCG@10 `+0.000118345582923`, recall@100 `+0.000259403173951`. The capped s40 serving smoke in `runs/eos-default-embedder-serving-smoke-20260620T161633Z/` remains predecessor proxy evidence. CorkScrewDB local flat packed-parent API evidence is now refreshed for the promoted narrow default: `runs/eos-narrow-default-corkscrewdb-budget-quality-packed-q4q8-main-20260623T050216Z` uses fresh SciFact vectors from `runs/eos-vector-caches/eos-narrow-default-scifact-child-w128-o32-128d` (`5,183` parents, `12,468` child vectors, `300` queries, `128d`, CUDA), layout `packed_parent_multivectors`, `metadata=none`, ordinal child IDs, `quantized_only` storage, and flat exact parent search. q4 measured nDCG@10 `0.452380`, recall@100 `0.758556`, DB directory multiple `0.041676x`, vector payload multiple `0.013312x`, p95 `16.076127ms`, planner fit `180`, target fit `true`, and target storage multiple `0.554545x`. q8 is diagnostic only: nDCG@10 `0.472132`, recall@100 `0.773556`, DB directory multiple `0.066734x`, vector payload multiple `0.025841x`, p95 `36.113081ms`, planner fit `93`, target fit `false`, and target storage multiple `1.074026x`. Current-cache HNSW/raw evidence is also refreshed in `runs/eos-current-default-corkscrewdb-hnsw-refresh-v1-20260623T055809Z`: q4 overfetch100 measured nDCG@10 `0.445689`, recall@100 `0.735222`, raw HNSW DB directory multiple `0.243494x`, vector payload multiple `0.013312x`, p95 `2.324070ms`, planner fit `123`, and target fit `true`; q4 overfetch12468 measured nDCG@10 `0.452380`, recall@100 `0.758556`, and p95 `49.635238ms`. q8 HNSW was not run. Full uncapped BM25-dot SearchMulti hybrid rows also pass for SciFact, NFCorpus, and FiQA. These rows cover local flat packed-parent, local HNSW/raw, and local hybrid API surfaces only; they are not remote mode, federation, hosted parity, or a service SLO.

Current capped LongEmbed evidence for the same s40 line is diagnostic and not a product-quality claim. The current promoted default preserves the synthetic late-needle wedge in `runs/eos-current-default-long-context-chunked-baseline-v1-20260623T055729Z/`: direct Eos nDCG@10 `0.119502020`, single-vector cache `0.157617787`, and chunked dense/q2/q4/q8 child-max rows at nDCG@10 and recall@100 `1.0`. This is synthetic smoke only, not product proof. The current promoted-default capped repo-docs cache comparison in `runs/eos-current-default-long-context-wedge-cache-v1-20260623T055955Z/` has `quality_claim=false` for every row: Eos direct nDCG@10 `0.634327008`, Eos token-span dense/q4 `0.656691634`/`0.639676979`, best Eos fusion `0.638514924`, Qwen3 dense/q4 `0.807649216`/`0.735872282`, and mxbai dense/q4 `0.783370960`/`0.750464207`, with recall@100 saturated at `1.0`. The official capped doc20/query20 external-cache blocker is cleared for `qmsum` and `2wikimqa`, and the v2 cache-mode rows preserve `quality_claim=false`. On `qmsum`, Eos direct nDCG@10 is `0.517955842`, best Eos fusion is `0.536813696`, Qwen3 128d q4 child is `0.876293470`, and mxbai 128d q4 child is `0.806946535`. On `2wikimqa`, the v2 external-only run has Eos direct `0.739153782`, best Eos fusion `0.744085103`, Qwen3 q4 `1.000000000`, and mxbai q4 `1.000000000`; the later sparse-enabled rerun's best Eos fusion is `0.739699216`, so cite the run used. The promoted-package sidecar retarget proof is `runs/eos-current-default-retarget-official-longembed-v1-qmsum-20260623T063632Z/`: QMSum doc20/query20, max observed document tokens `4096`, Eos direct `0.517955842`, token-span dense/q4 `0.547133297`/`0.567516406`, best Eos fusion `0.540547745`, Qwen3 q4 `0.848162307`, and mxbai q4 `0.828480363`; all rows are `quality_claim=false`, recall@100 saturates, and 2WikiMultihopQA was not rerun for this retarget slice. Sparse encoder parent rows are present and subquadratic in the host-reference audit, but are quality-negative versus direct on both datasets: `qmsum` sparse dense/q4 `0.458166919`/`0.420963072`, and `2wikimqa` sparse dense/q4 `0.504805535`/`0.505901435`. Preserve `quality_claim=false`, capped doc20/query20 caveats, external-cache caveats, host-reference sparse-encoder caveats, capped repo-docs caveats, and external child-count/storage caveats when using these rows. The next long-context claim needs a changed objective/model or stronger comparable real-doc Eos rows before any product-quality claim.

Dense-quality next lane status: `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` generated the older request-only NFCorpus train/dev frontier and mechanically unblocked true-reranker scoring through `cmd/teacher-bridge --mode tei-rerank` against TEI `/rerank`; RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9`. The broader selected-diverse BGE audit at `runs/eos-nfcorpus-broader-cleaned-mining-quality-signal-v1-20260623T101157Z/` remains historical audit-only evidence: BGE large was strongest on that slice at `65/128` (`0.507812`), but the trainability review stayed `no_train` because agreement and margins were too weak.

The cleaned, provenance-safe full frontier at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/` closes Jina and BGE-large as reranker-teacher lanes for this frontier, and the mxbai-base subset now closes the immediate mxbai-base trainability question. Jina v2 scored `7128/7128` request rows and selected the positive at top-1 for only `160/1188 = 0.134680`. `BAAI/bge-reranker-large` then reused the same cleaned frontier, existing TEI cache, and no-download setup; its debug/artifacts are under `debug-bge-large-full-clean-v1/` and `data/nfcorpus.train-dev.cleaned-full-frontier.bge-large.*`. BGE-large scored `7128/7128` request rows in `1:03.02` with `duplicates_skipped=0`, imported `1188` examples with missing `0`, audited `1188` scored / `0` missing / `0` duplicate-positive negatives, and selected only `142/1188 = 0.119529` positives at top-1. Mean normalized entropy was `0.993488`; the filter kept `142` and cleared `1046`. `mixedbread-ai/mxbai-rerank-base-v2` still failed TEI `120-1.9` serving, but HF/SentenceTransformers CrossEncoder scoring is mechanically unblocked in `runs/eos-mxbai-rerank-base-hf-unblock-smoke-v1-20260623T124344Z`; the run-local HF cache was about `958M` and final free space was about `33G`. Its tiny raw-logit smoke passed, then the deterministic cleaned-frontier subset64 scored/imported/audited/filtered with positive top1 `1/64 = 0.015625`, mean margin `-7.836426`, mean normalized entropy `0.476268`, filter kept `1`, and cleared `63`. Because the subset failed the `0.35` gate, full mxbai-base scoring was skipped and there is no full-frontier mxbai-base metric. `mixedbread-ai/mxbai-rerank-large-v2` was later scored through HF/SentenceTransformers raw logits in `runs/eos-mxbai-rerank-large-hf-subset-audit-v1-20260623T140353Z`, using the repaired subset from `runs/eos-nfcorpus-repaired-frontier-subset64-v1-20260623T131418Z/`. Its smoke passed at relevant `12.25` and irrelevant `-0.5625`; repaired subset64 passed at `37/64 = 0.578125`, mean margin `1.179688`, norm entropy `0.442453`, kept `37/64`, but the full cleaned frontier scored all `7128` request rows over `1188` examples with missing `0` and duplicate-positive negative candidates `0`, then failed at `172/1188 = 0.144781`, mean margin `-2.897175`, norm entropy `0.612920`, kept `172/1188`. Mark Jina, BGE-large, mxbai-base subset, and mxbai-large full-frontier evidence `teacher_quality_blocked` / `no_train`; mark the mxbai-base run specifically `subset_quality_gate_failed`. Generated rows remain `request_only=true` plus `train_allowed=false`; do not train or promote from these artifacts. The next dense-quality action should pivot to capacity/bootstrap, a compact/Matryoshka objective, or a different data strategy.

Repaired NFCorpus subset64 BGE resmoke evidence is recorded at `runs/eos-nfcorpus-repaired-subset64-bge-resmoke-v1-20260623T132032Z`. The source-label repair improved selected-positive top1 versus the prior cleaned subset64 BGE rows, but the evidence remains `no_train` / audit-only: every generated row is `request_only=true` and `train_allowed=false`, duplicate/qrels-positive negative leaks are zero, selected-positive and any-positive top1 are identical, and no source promotion or training should use these artifacts.

| subset64 teacher | runtime | selected/any-positive top1 | mean margin | norm entropy | kept |
| --- | --- | ---: | ---: | ---: | ---: |
| `BAAI/bge-reranker-base` | TEI `120-1.9`, repaired subset | `35/64` (`0.546875`) | `0.216738` | `0.980823` | `35/64` |
| `BAAI/bge-reranker-large` | TEI `120-1.9`, repaired subset | `33/64` (`0.515625`) | `0.215547` | `0.983283` | `33/64` |
| `BAAI/bge-reranker-v2-m3` | TEI `120-1.9`, repaired subset | `33/64` (`0.515625`) | `0.194146` | `0.979424` | `33/64` |
| `mixedbread-ai/mxbai-rerank-base-v2` | HF/SentenceTransformers CrossEncoder raw logits, cleaned subset | `1/64` (`0.015625`) | `-7.836426` | `0.476268` | `1/64` |
| `mixedbread-ai/mxbai-rerank-large-v2` | HF/SentenceTransformers CrossEncoder raw logits, repaired subset | `37/64` (`0.578125`) | `1.179688` | `0.442453` | `37/64` |
| `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` | historical dirty subset only; Python seq-cls fallback; TEI `120-1.9` unsupported for Qwen3 classifier | `5/64` (`0.078125`) | `-0.5452850256115198` | - | `5/64` |

BGE dirty-to-cleaned subset64 selected-top1 changes were base `+2`, large `+7`, and v2-m3 `+1`; the repaired source-label subset then improved the cleaned BGE rows from base `29->35`, large `22->33`, and v2-m3 `10->33`. The repaired subset64 BGE audits report `duplicate_positive_negative_candidates=0`, qrels-positive negative leaks `0`, and equal selected/any-positive top1 because no exact-text duplicate negative remained qrels-positive. The broader 128-row audit supersedes prior cleaned subset64 as BGE evidence, while the repaired subset64 row records a better bounded diagnostic and reaches the same no-train conclusion. Broader kept-example agreement was `0` models: `49`, `1` model: `17`, `2` models: `20`, `3` models: `42`. The mxbai-base subset64 row is subset-only and must not be compared as a full-frontier metric; no full mxbai-base score exists. The mxbai-large subset row passed, but its full-frontier top1 was only `172/1188 = 0.144781` with negative mean margin, so it also remains `teacher_quality_blocked` / `no_train`. Qwen3 was not rerun on cleaned or repaired data and should not be cited as cleaned-data evidence. Metadata preservation through import/filter was verified on subset64 after `a7482cc`; the repaired evidence remains audit-only/no-train until a teacher passes positive-top1, margin, novelty, and leakage gates on the full frontier.
...
| s40 dense candidate | 0.564538 | 0.205571 | 0.121261 | 0.297123 | 0.463394 | predecessor default |
| s40 NFCorpus compact-mined narrow | 0.564538 | 0.205746 | 0.121261 | 0.297182 | 0.463396 | current promoted default |
...
On RTX50/Blackwell hosts, use `ghcr.io/huggingface/text-embeddings-inference:120-1.9`; the generic `cuda-1.9` image reached CUDA model initialization in the local probe but did not become healthy. The `120-1.9` image became healthy and served true `/rerank` for `BAAI/bge-reranker-v2-m3` and `BAAI/bge-reranker-large`.
...
After scoring it with a real reranker service, import the scored rows with `eos import-teacher-scores`, then run `audit-teacher-scores` and `filter-teacher-scores`. When qrels and corpus provenance exist, run `audit-teacher-scores --qrels <qrels.tsv> --corpus <corpus.jsonl> <scored-hard-negatives.jsonl> <summary.json>` to add duplicate/multi-positive-aware diagnostics while preserving the selected-positive defaults. Training remains blocked until real scorer output passes positive-top1, margin, novelty, and leakage gates; fake scorer or embedding-cosine bridge output is only plumbing evidence. The broader selected-diverse BGE audits remain historical no-train evidence; BGE-large was strongest there at `65/128` (`0.507812`), but agreement and margins were too weak.

Jina v2 HF fallback, BGE-large TEI, and mxbai-large HF/SentenceTransformers are now negative full-frontier model-quality evidence. On the cleaned full-frontier rebuild at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/`, Jina collapsed to `160/1188 = 0.134680` selected/any-positive top1. On the same cleaned/provenance-safe frontier, `BAAI/bge-reranker-large` scored `7128/7128` request rows in `1:03.02` with `duplicates_skipped=0`, imported `1188` examples with missing `0`, audited `1188` scored / `0` missing / `0` duplicate-positive negatives, and selected only `142/1188 = 0.119529` positives at top-1. Mean normalized entropy was `0.993488`; the filter kept `142` and cleared `1046`. BGE artifacts are under `debug-bge-large-full-clean-v1/` and `data/nfcorpus.train-dev.cleaned-full-frontier.bge-large.*`; the existing TEI cache was reused, no download was needed, the container was cleaned up, and final worker status was clean. `mixedbread-ai/mxbai-rerank-base-v2` remains TEI-blocked, but HF/SentenceTransformers CrossEncoder scoring was mechanically unblocked in `runs/eos-mxbai-rerank-base-hf-unblock-smoke-v1-20260623T124344Z`. Its tiny raw-logit smoke passed; the cleaned-frontier subset64 then scored/imported/audited/filtered at positive top1 `1/64 = 0.015625`, mean margin `-7.836426`, mean normalized entropy `0.476268`, filter kept `1`, and cleared `63`. The subset failed the `0.35` gate, so full scoring was skipped and there is no full-frontier mxbai-base metric. `mixedbread-ai/mxbai-rerank-large-v2` was scored in `runs/eos-mxbai-rerank-large-hf-subset-audit-v1-20260623T140353Z` from the repaired subset source `runs/eos-nfcorpus-repaired-frontier-subset64-v1-20260623T131418Z/`; its smoke passed at relevant `12.25` and irrelevant `-0.5625`, repaired subset64 reached `37/64 = 0.578125` with margin `1.179688`, norm entropy `0.442453`, and kept `37/64`, then full cleaned-frontier scoring covered `7128` request rows / `1188` examples with missing `0` and duplicate-positive negative candidates `0` but failed at `172/1188 = 0.144781`, mean margin `-2.897175`, norm entropy `0.612920`, kept `172/1188`. Mark Jina, BGE-large, this mxbai-base subset, and mxbai-large full-frontier evidence `teacher_quality_blocked` / `no_train`, with mxbai-base specifically `subset_quality_gate_failed`; do not train from these rows. `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` was not rerun on cleaned data; its historical dirty-subset audit stayed no-train at positive-top1 `0.078125`, kept `5/64`, mean margin `-0.5452850256115198`. The next descriptor should pivot to capacity/bootstrap, a compact/Matryoshka objective, or a different data strategy.
...
MS MARCO hard-candidate/dual-teacher status: research-only lineage, with `release_train_allowed=false`, `commercial_use_allowed=false`, and `train_allowed_for_research=true`. The hard-candidate substrate has `5,000` rows and `135,565` negatives from bounded BM25, lexical-overlap, and random controls; the current-Eos full-corpus lane is blocked by no local MS MARCO cache. Qwen3 hard scoring reached positive top1 `0.6108`; mxbai hard scoring reached positive top1 `0.6626`. The clean agreement split has `2,379` rows with no global dev-positive negative refs; `teacher_scores` are averaged per-row z-score Qwen3/mxbai vectors, and audit reports positive_top1 `1.0`, mean margin `2.191931`, and entropy `0.599264`. Official `Qwen/Qwen3-Reranker-0.6B` revision `e61197ed45024b0ed8a2d74b80b4d909f1255473` was downloaded and used from a local Hugging Face cache of about `1207488344` bytes; the known-bad `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` path was not used. It scored the prepared `2,048` request rows in `29.666s` (`69.036` req/s) on CUDA bfloat16 batch `8` using yes/no final-token logits. Over the `512` conflict rows, outcomes were `alternate_relevant=253`, `ambiguous=141`, and `confirmed_ordering=118`. All `5` zero-margin fixtures were present: `2` distinct-text ties were confirmed, while `3` normalized duplicate-text ties stayed ambiguous. The completed score-spectrum artifact has `2,891` rich rows (`2,379` clean + `512` conflict) and `2,497` current-trainer preview rows, with legal gates false/false/true preserved. Alternate-relevant rows are preserved as multi-positive soft-target rows, not forced into the current hard-negative trainer format.

MS MARCO clean-agreement LR-control status: closed as no-advance. The prior LR `2.5e-8` smoke moved embeddings but regressed AUC/margin, so the single allowed control selected `1.25e-8`. The first LR-control wrapper died during eval under memory pressure, but eval salvage with batch `8` succeeded. The `1.25e-8` package exactly matched matched-bootstrap final/hard eval metrics, produced no top1/MRR lift, and movement diagnostics showed zero changed vectors at 64d, 128d, and full dimensions. Do not spend more on strict clean-agreement LR tuning, retrieval scoreboard, or compact scoring for this branch. Do not train from the score-spectrum preview as the main quality path because it excludes the `253` alternate-relevant rows; the next engineering move is research-only trainer support for multi-positive/soft-target score-spectrum rows.
...
Status: local process-doc pretraining now has a first landing zone in `scripts/build_pretrain_pairs.fw`. Set `EOS_PROCESS_PRETRAIN=1` to add chunks from `AGENTS.md`, `.codex/agents/*.toml`, and `.codex/skills/**/SKILL.md` to the hard-negative pretraining JSONL; set `EOS_PROCESS_PRETRAIN_INCLUDE_DOCS=1` to include `docs/**/*.md`. Set `EOS_PROCESS_PRETRAIN_SYNTHETIC_QUERIES=1` to replace generic `process guidance in <path>` rows with deterministic path-, heading-, and term-derived query-style rows under `process-query:*` sources. The output uses the existing text hard-negative fields (`query`, `positive`, `negatives`, `source`, `group_id`), so it can be blended into `processed/pretrain-pairs.jsonl` before the shipping pipeline or used directly with `EOS_HARD_NEGATIVE_TRAIN=1` for a candidate smoke. A bounded process-corpus smoke generated `12` process rows, reached hard-negative training with `optimizer_updates=42`, and completed a separate eval-only pass with `optimizer_updates=0`; this proves the plumbing path only, not model quality. Synthetic process queries are generated-data plumbing/proof until downstream retrieval or guarded training shows model-quality movement.
...
New trainable sidecar scaffold: `eos fit-sparse-lexical-linear-head` writes experimental `manta.sparse_lexical_linear_head.v1` artifacts from sparse lexical labels plus dense doc/query vector caches. `eos eval-sparse-lexical-linear-head-vectors-hybrid` predicts retained hashed sparse bins from dense vectors and fuses them with dense scores without loading eval-time sparse labels. This materially separates the next quality lane from split-specific label sidecars while remaining non-default, experimental sidecar evidence only.

Linear-head calibration controls are also available as experimental, non-default probes: `--target-transform identity|log1p` at fit time, and eval-time `--doc-max-terms`, `--query-max-terms`, and `--score-threshold`. The SciFact calibration smoke at `runs/eos-sparse-linear-head-calibration-scifact-smoke-v1-20260622T121118Z/` found that `log1p` helps sparse-only scoring versus identity but does not make the trained linear sparse head product-useful. Best sparse-only was `sparse_log1p8192_default`, nDCG@10 `0.209002` and recall@100 `0.539556`. Best hybrid was `hybrid_zscore_a0p25_dco_protect5_log1p2048_default`, nDCG@10 `0.561401` and recall@100 `0.796444`, below dense baseline nDCG@10 `0.564538` and below the prior best linear-head smoke nDCG@10 `0.563870`. Query caps and thresholds hurt in this smoke. Stop expanding this linear-head calibration lane unless a new mechanism directly addresses the low-overlap/common-bin-prior failure mode.
...
Reusable calibration scaffold: `scripts/calibrate_eos_sparse_lexical_head_hybrid.fw` now productizes the split-specific sparse lexical hash-head sidecar workflow. It exports per-split sparse labels, fits the experimental hash-head sidecar, exports dense vectors, sweeps configured fusion settings on dev, applies the selected setting unchanged to test, and writes JSON/TSV/Markdown summaries under `runs/<run-id>/`.

Experimental learned/prototype scaffold: `fit-sparse-lexical-projection-head` writes `manta.sparse_lexical_projection_head.v1` artifacts by hashing sparse labels into bins, accumulating label-weighted dense-vector prototypes for those bins, normalizing the prototypes, and storing bounded prediction metadata (`dimension`, `hash_bins`, `max_prototypes`, `max_predicted_terms`, normalization). `eval-sparse-lexical-projection-head-vectors-hybrid` then predicts sparse bins from dense doc/query vectors and evaluates sparse-only or dense+sparse fusion without loading eval-time sparse labels; the fit split recorded in the head may differ from the held-out eval qrels/corpus split. Its `--dense-candidates-only` flag is an experimental recall-preserving rerank mode for projection evidence: projected sparse scores can reorder dense top-k candidates, but sparse-only candidates cannot enter the result set or displace dense tail candidates. Prototype retention ranking is configurable with `support|total_weight|avg_weight`; `support` remains the default, and weight-aware policies are probes. This is the first reusable sidecar direction beyond split-specific label scoring, but remains experimental evidence only.

Updated projection-head capacity evidence lives at `runs/eos-projection-head-capacity-term-sweep-v1-20260622T094939Z/`, with weight-aware policy validation at `runs/eos-projection-head-weight-aware-ranking-v1-20260622T104100Z/` and full-FiQA validation at `runs/eos-projection-head-fiqa-full-validation-v1-20260622T105155Z/`. The strongest full rows remained under the default `support` policy: SciFact full `p8192_t32 support zscore alpha0.25 protect5`, nDCG@10 `0.568415`, R@100 `0.796444`, dense `0.564538`; NFCorpus full `p8192_t32 support zscore alpha0.25 protect0`, nDCG@10 `0.210277`, R@100 `0.242059`, dense `0.205571`. FiQA lift was capped exploratory evidence only: `max_queries_100` `p512_t128 support alpha0.30`, nDCG@10 `0.104322`, R@100 `0.291298`, dense maxq100 `0.098636`. Full FiQA did not beat dense: the best full row was `p512_t32 support alpha0.25`, nDCG@10 `0.120537` versus dense `0.121261`, with recall@100 matching `0.351678`. `avg_weight` underperformed, and `total_weight` had only a tiny NFCorpus lift. This is evidence for recall-preserving projection reranking research rather than default policy, asset promotion, or a claim that projected sparse candidates should replace dense candidate coverage.

Full uncapped short-set calibration from that scaffold is reported at `runs/eos-sparse-head-hybrid-calibration-shortset-full-v1-20260622T080053Z/` and `.tiller/scratch/codex/eos-sparse-head-hybrid-calibration-shortset-full-v1-report.md`. SciFact used train-as-dev fallback and selected/fixed `hybrid_zscore_alpha0.25`, reaching test nDCG@10 / recall@100 / P@1 `0.657217 / 0.928889 / 0.570000` versus dense `0.564538 / 0.796444 / 0.476667`. NFCorpus selected `hybrid_minmax_alpha0.75`, reaching `0.307467 / 0.284291 / 0.399381` versus dense `0.205571 / 0.242059 / 0.263158`; FiQA selected the same setting, reaching `0.214599 / 0.487663 / 0.175926` versus dense `0.121261 / 0.351678 / 0.092593`. The fixed global `hybrid_zscore_alpha0.25` row and the dev-selected row are positive versus dense everywhere, all protection gates passed, and reported missing sparse labels are zero. This is still experimental split-specific hashed sparse-label sidecar evidence, not a trained neural sparse head, dense model promotion, default asset change, or alias change; FiQA dense/hybrid still skips relevant docs without text (`2` dev, `1` test), while sparse labels cover them as zero-term labels.

Next scaffold: train an actual neural sparse head instead of relying on split-specific lexical-label sidecars. Keep these runs as separate experimental sidecar artifacts under `runs/`, keep `.mll` default output and shipped assets unchanged, and require quality proof before any alias/default discussion. Minimum gate: dense+sparse must beat dense-only on held-out data without violating per-dataset floors, and reports must state whether gains come from trained sparse output or from lexical labels/oracle baselines.
