...
Dense-quality next lane status: `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` generated a request-only NFCorpus train/dev frontier for stronger reranker teaching, with `1,188` hard-negative examples and `7,066` scorer request rows. Decision is `no_train` / `scorer_blocked`: no local stronger reranker or cross-encoder query-document scorer was found, embedding/cosine or vector-cache teachers are not a substitute for this lane, and the generated rows have `request_only=true` plus `train_allowed=false`. Wire or acquire a true cross-encoder/reranker scorer, score the request manifest, then rerun positive-top1, margin, novelty, and leakage gates before proposing any dense training.
