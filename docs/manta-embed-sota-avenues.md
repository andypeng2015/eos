...
Dense-quality next lane status: `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` generated the older request-only NFCorpus train/dev frontier and mechanically unblocked true-reranker scoring through `cmd/teacher-bridge --mode tei-rerank` against TEI `/rerank`; RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9`. The broader selected-diverse BGE audit at `runs/eos-nfcorpus-broader-cleaned-mining-quality-signal-v1-20260623T101157Z/` remains audit-only: BGE base selected/any-positive top1 was `59/128` (`0.460938`), BGE large `65/128` (`0.507812`), and BGE v2-m3 `59/128` (`0.460938`), with `duplicate_positive_negative_candidates=0` for each model. BGE large is the strongest BGE signal on this slice, but the trainability review remains `no_train`: all-three agreement kept only `42/128`, at-least-two agreement kept `62/128`, all-three is too small/correlated, and at-least-two margins are too weak. The MiniLM control also remains no-train at `54/128` (`0.421875`) selected/any-positive top1 with selected/any mean margin `0.140112`.

Jina v2 scored through local Hugging Face fallback after TEI config parsing failed. The optimistic 128-row selected-diverse result (`67/128 = 0.523438`) did not generalize. The cleaned, provenance-safe full-frontier audit at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/` selected `1188` examples (`932` train + `256` dev), scored `7128/7128` request rows, imported `1188/1188` examples with missing `0`, and passed clean validation: doc-id provenance present, duplicate qrels/text positive negatives `0`, duplicate `(source, query, candidate)` request keys `0`, and selected unscored `teacher_scores` rows `0`. Jina selected/any-positive top1 was only `160/1188 = 0.134680`, mean margin `-1.182826`, and mean normalized entropy `0.883789`; the filter kept `160`, cleared `1028`, and dropped `0`. Train split top1 was `122/932 = 0.130901`; dev split was `38/256 = 0.148438`. Mark Jina `teacher_quality_blocked` / `no_train`; no guarded trainability descriptor should be proposed from it. This supersedes the old dirty full-frontier caveat: contamination was not the only blocker, because Jina still failed teacher-quality gates after the clean doc-id-provenance rebuild. `mixedbread-ai/mxbai-rerank-base-v2` and `mixedbread-ai/mxbai-rerank-large-v2` still failed TEI `120-1.9` serving even with `--pooling cls` / `--pooling mean` because `sentence_bert_config.json.max_seq_length` is missing; treat those as compatibility blockers, not model-quality evidence. Generated rows remain `request_only=true` plus `train_allowed=false`; do not train or promote from these artifacts. The next dense-quality action should pivot away from Jina toward BGE-large/mxbai compatibility, synthetic or non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka objectives, or larger-model/bootstrap.
...
After scoring it with a real reranker service, import the scored rows with `eos import-teacher-scores`, then run `audit-teacher-scores` and `filter-teacher-scores`. When qrels and corpus provenance exist, run `audit-teacher-scores --qrels <qrels.tsv> --corpus <corpus.jsonl> <scored-hard-negatives.jsonl> <summary.json>` to add duplicate/multi-positive-aware diagnostics while preserving the selected-positive defaults. Training remains blocked until real scorer output passes positive-top1, margin, novelty, and leakage gates; fake scorer or embedding-cosine bridge output is only plumbing evidence. Current broader selected-diverse BGE true-reranker audits remain no-train: `BAAI/bge-reranker-base` selected/any-positive top1 `59/128` (`0.460938`), `BAAI/bge-reranker-large` `65/128` (`0.507812`), and `BAAI/bge-reranker-v2-m3` `59/128` (`0.460938`), all with `duplicate_positive_negative_candidates=0`. Agreement is not trainable: all-three kept `42/128`, at-least-two kept `62/128`, all-three is too small/correlated, and at-least-two margins are too weak. The completed non-BGE MiniLM control also remains no-train at `54/128` (`0.421875`) with selected/any mean margin `0.140112`, `48` kept intersection with BGE-large, and top1 candidate agreement `81/128`.

Jina v2 HF fallback is now negative model-quality evidence. The 128-row selected-diverse slice reached selected/any-positive top1 `67/128` (`0.523438`), but the cleaned full-frontier rebuild at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/` collapsed to `160/1188 = 0.134680` selected/any-positive top1, mean margin `-1.182826`, mean normalized entropy `0.883789`, filter kept `160`, cleared `1028`, and dropped `0`. The rebuilt frontier selected `1188` examples (`932` train + `256` dev), scored `7128/7128` request rows, imported `1188/1188` examples, and validated cleanly with doc-id provenance present, duplicate qrels/text positive negatives `0`, duplicate `(source, query, candidate)` request keys `0`, and selected unscored `teacher_scores` rows `0`. Train split top1 was `122/932 = 0.130901`; dev split was `38/256 = 0.148438`. Mark Jina full frontier `teacher_quality_blocked` / `no_train`; do not train from these rows. The old contamination caveat is no longer the main decision: Jina still fails after the provenance-safe rebuild. mxbai-rerank-base-v2 and large-v2 failed TEI `120-1.9` serving even with `--pooling cls` / `--pooling mean` because `sentence_bert_config.json.max_seq_length` is missing. These are compatibility blockers, not model-quality evidence. Cleaned subset64 BGE remains historical initial evidence, not the latest BGE audit. `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` was not rerun on cleaned data; its historical dirty-subset audit stayed no-train at positive-top1 `0.078125`, kept `5/64`, mean margin `-0.5452850256115198`. Qwen3 seq-cls TEI `120-1.9` startup failed because `classifier model type is not supported for Qwen3`; the Python `AutoModelForSequenceClassification` fallback on CUDA RTX 5070 Ti was mechanically scoreable with Qwen3 model-card formatting, but it also failed the quality gate. The next descriptor should pivot away from Jina toward BGE-large/mxbai compatibility, synthetic or non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka objectives, or larger-model/bootstrap.
