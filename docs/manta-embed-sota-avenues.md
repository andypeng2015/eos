...
MS MARCO hard-candidate/dual-teacher status: research-only lineage, with `release_train_allowed=false`, `commercial_use_allowed=false`, and `train_allowed_for_research=true`. The hard-candidate substrate has `5,000` rows and `135,565` negatives from bounded BM25, lexical-overlap, and random controls; the current-Eos full-corpus lane is blocked by no local MS MARCO cache. Qwen3 hard scoring reached positive top1 `0.6108`; mxbai hard scoring reached positive top1 `0.6626`. The clean agreement split has `2,379` rows with no global dev-positive negative refs; `teacher_scores` are averaged per-row z-score Qwen3/mxbai vectors, and audit reports positive_top1 `1.0`, mean margin `2.191931`, and entropy `0.599264`. The strict conflict queue has `2,200` rows. Zero-margin edge inspection found `5` rows: `3` normalized-identical text ties and `2` distinct-text scorer ties, with no duplicate doc IDs, no qrels-positive leaks, and exact source JSON float ties. Keep those rows as regression fixtures, not forced labels. Reranker staging prepared a deterministic stratified `512`-row slice and `2,048` request rows, force-including all five zero-margin fixtures and preserving legal gates false/false/true. No official offline `Qwen/Qwen3-Reranker-*` model was cached, so no reranker labels were produced; available cached models were `Qwen/Qwen3-Embedding-0.6B` and `mixedbread-ai/mxbai-embed-large-v1`, and the known-bad `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` path was not used.

MS MARCO clean-agreement LR-control status: closed as no-advance. The prior LR `2.5e-8` smoke moved embeddings but regressed AUC/margin, so the single allowed control selected `1.25e-8`. The first LR-control wrapper died during eval under memory pressure, but eval salvage with batch `8` succeeded. The `1.25e-8` package exactly matched matched-bootstrap final/hard eval metrics, produced no top1/MRR lift, and movement diagnostics showed zero changed vectors at 64d, 128d, and full dimensions. Do not spend more on strict clean-agreement LR tuning, retrieval scoreboard, or compact scoring for this branch. Next action is to make an official Qwen3 reranker available offline and score the prepared `512`-row slice, or build a score-spectrum artifact only after adjudication.
