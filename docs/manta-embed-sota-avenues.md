...
Dense-quality next lane status: `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` generated a request-only NFCorpus train/dev frontier for stronger reranker teaching, with `1,188` hard-negative examples and `7,066` scorer request rows. The true-reranker scorer path is mechanically unblocked: `cmd/teacher-bridge --mode tei-rerank` works against TEI `/rerank`, and RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9` rather than the generic CUDA image. The newer broader selected-diverse BGE audit at `runs/eos-nfcorpus-broader-cleaned-mining-quality-signal-v1-20260623T101157Z/` is audit evidence only, not a training gate: BGE base selected/any-positive top1 was `59/128` (`0.460938`), BGE large `65/128` (`0.507812`), and BGE v2-m3 `59/128` (`0.460938`), with `duplicate_positive_negative_candidates=0` for each model. BGE large is the strongest BGE signal on this slice, but the trainability review remains `no_train`: all-three agreement kept only `42/128`, at-least-two agreement kept `62/128`, all-three is too small/correlated, and at-least-two margins are too weak. The completed non-BGE MiniLM control also remains no-train: `cross-encoder/ms-marco-MiniLM-L6-v2` selected/any-positive top1 `54/128` (`0.421875`), kept `54/128`, duplicate-positive negative candidates `0`, selected/any mean margin `0.140112`, mean normalized entropy `0.985032`, and filter mean margin after `0.452819`. It is weaker than BGE-large `65/128` and overlaps heavily with it (`48` kept intersection, top1 candidate agreement `81/128`), so the frontier stays `no_train` / `teacher_quality_blocked`. Jina v2 scored through local Hugging Face fallback after TEI config parsing failed: the 128-row selected-diverse slice reached selected/any-positive top1 `67/128` (`0.523438`), selected mean margin `0.366817`, mean normalized entropy `0.813294`, and kept `67/128`, but this optimistic small-slice result does not generalize to the full frontier. Full-frontier Jina HF scored `7066/7066` request rows and imported `1188/1188` examples after a score/import-dedup sidecar for 8 duplicate `(source, query, candidate)` keys. Full selected-positive top1 was `158/1188` (`0.132997`), mean margin `-1.110194`, mean normalized entropy `0.873741`, filter kept `158/1188`, train split `108/932` (`0.115880`), and dev split `50/256` (`0.195312`). Mark Jina full frontier `teacher_quality_blocked` / `no_train`; no guarded trainability descriptor should be proposed from it. Doc-id provenance caveat: qrels any-positive diagnostics could not materialize on the old full frontier because examples lack `query_id`, `positive_doc_id`, and `negative_doc_ids`; duplicate selected-positive-as-negative rows also exist in the full frontier request file. `mixedbread-ai/mxbai-rerank-base-v2` and `mixedbread-ai/mxbai-rerank-large-v2` failed TEI `120-1.9` serving even with `--pooling cls` / `--pooling mean` because `sentence_bert_config.json.max_seq_length` is missing. Treat those as TEI compatibility blockers, not model-quality evidence. Generated rows remain `request_only=true` plus `train_allowed=false`; do not train or promote from these artifacts. The next dense-quality action is to rebuild/remine the full NFCorpus reranker frontier using the cleaned duplicate-positive filters and doc-id provenance now available, then re-audit any teacher; alternatives are synthetic/non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka signal, or larger-model/bootstrap. Do not train from the old full-frontier artifacts.

| subset64 teacher | runtime | selected/any-positive top1 | kept |
| --- | --- | ---: | ---: |
| `BAAI/bge-reranker-base` | TEI `120-1.9`, cleaned subset | `29/64` (`0.453125`) | `29/64` |
| `BAAI/bge-reranker-large` | TEI `120-1.9`, cleaned subset | `22/64` (`0.343750`) | `22/64` |
| `BAAI/bge-reranker-v2-m3` | TEI `120-1.9`, cleaned subset | `10/64` (`0.156250`) | `10/64` |
| `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` | historical dirty subset only; Python seq-cls fallback; TEI `120-1.9` unsupported for Qwen3 classifier | `5/64` (`0.078125`) | `5/64` |

BGE dirty-to-cleaned subset64 selected-top1 changes were base `+2`, large `+7`, and v2-m3 `+1`; the cleaned subset64 BGE audits report `duplicate_positive_negative_candidates=0` and equal selected/any-positive top1 because no exact-text duplicate negative remained qrels-positive. The broader 128-row audit supersedes subset64 as BGE evidence, but preserves subset64 as the initial cleaned-data diagnostic and reaches the same no-train conclusion. Broader kept-example agreement was `0` models: `49`, `1` model: `17`, `2` models: `20`, `3` models: `42`. Qwen3 seq-cls mean margin on the earlier dirty subset was `-0.5452850256115198`; filtering kept `5/64`, cleared `59`, dropped `0`, and preserved request-only/no-train metadata. Qwen3 was not rerun on cleaned data and should not be cited as cleaned-data evidence. Metadata preservation through import/filter was verified on subset64 after `a7482cc`; the evidence remains audit-only/no-train until a non-BGE teacher passes positive-top1, margin, novelty, and leakage gates.
...
After scoring it with a real reranker service, import the scored rows with `eos import-teacher-scores`, then run `audit-teacher-scores` and `filter-teacher-scores`. When qrels and corpus provenance exist, run `audit-teacher-scores --qrels <qrels.tsv> --corpus <corpus.jsonl> <scored-hard-negatives.jsonl> <summary.json>` to add duplicate/multi-positive-aware diagnostics while preserving the selected-positive defaults. Training remains blocked until real scorer output passes positive-top1, margin, novelty, and leakage gates; fake scorer or embedding-cosine bridge output is only plumbing evidence. Current broader selected-diverse BGE true-reranker audits remain no-train: `BAAI/bge-reranker-base` selected/any-positive top1 `59/128` (`0.460938`), kept `59/128`; `BAAI/bge-reranker-large` `65/128` (`0.507812`), kept `65/128`; and `BAAI/bge-reranker-v2-m3` `59/128` (`0.460938`), kept `59/128`, all with `duplicate_positive_negative_candidates=0`. Agreement is not trainable: all-three kept `42/128`, at-least-two kept `62/128`, all-three is too small/correlated, and at-least-two margins are too weak. The completed non-BGE control with `cross-encoder/ms-marco-MiniLM-L6-v2` also remains no-train: selected/any-positive top1 `54/128` (`0.421875`), kept `54/128`, duplicate-positive negative candidates `0`, selected/any mean margin `0.140112`, mean normalized entropy `0.985032`, filter mean margin after `0.452819`, `48` kept intersection with BGE-large, and top1 candidate agreement `81/128`. Jina v2 HF fallback is now model-quality evidence: the 128-row selected-diverse slice reached selected/any-positive top1 `67/128` (`0.523438`), but the old full frontier collapsed to `158/1188` selected-positive top1 (`0.132997`), mean margin `-1.110194`, mean normalized entropy `0.873741`, filter kept `158/1188`, train split `108/932` (`0.115880`), and dev split `50/256` (`0.195312`) after scoring `7066/7066` rows and importing `1188/1188` examples through a score/import-dedup sidecar for 8 duplicate `(source, query, candidate)` keys. Mark Jina full frontier `teacher_quality_blocked` / `no_train`. Doc-id provenance caveat: qrels any-positive diagnostics could not materialize on the old full frontier because examples lack `query_id`, `positive_doc_id`, and `negative_doc_ids`; duplicate selected-positive-as-negative rows also exist in the full frontier request file. mxbai-rerank-base-v2 and large-v2 failed TEI `120-1.9` serving even with `--pooling cls` / `--pooling mean` because `sentence_bert_config.json.max_seq_length` is missing. These are compatibility blockers, not model-quality evidence. Cleaned subset64 BGE remains historical initial evidence, not the latest BGE audit. `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` was not rerun on cleaned data; its historical dirty-subset audit stayed no-train at positive-top1 `0.078125`, kept `5/64`, mean margin `-0.5452850256115198`. Qwen3 seq-cls TEI `120-1.9` startup failed because `classifier model type is not supported for Qwen3`; the Python `AutoModelForSequenceClassification` fallback on CUDA RTX 5070 Ti was mechanically scoreable with Qwen3 model-card formatting, but it also failed the quality gate. The next descriptor should rebuild/remine the full NFCorpus reranker frontier using the cleaned duplicate-positive filters and doc-id provenance now available, then re-audit any teacher; alternatives are synthetic/non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka signal, or larger-model/bootstrap. Do not train from the old full-frontier artifacts.
