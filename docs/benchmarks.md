...
For the accepted targeted-v3 compact policy, prefer the repo-local serving proxy:
...
It compares q4/fp16/overfetch250 with q8/fp16/overfetch125 and writes `summary.tsv` plus `manifest.json` under `runs/eos-default-embedder-serving-smoke-<timestamp>/`. This is not an actual CorkScrewDB API smoke; it is the in-repo TurboQuant serving proxy. For the EOS-named targeted-v3 release artifact, q4/fp16 overfetch250 fails strict zero-regression only on NFCorpus recall@100 by `-0.000236936880015165`; under the explicit `recall@100` tolerance `0.00025`, q4/fp16 overfetch250 is the compact release-candidate profile and total compression is `1.590062x`. Full uncapped SciFact serving proxy run `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/serving-smoke-scifact-full/` measured q4/fp16/250 at nDCG@10 `0.5623217863`, recall@100 `0.7964444444`, total compression `1.5900621118x`, p95 `7.607784 ms`, `414712.08` scores/s, and `75,000` rerank scores; q8/fp16/125 matched the same SciFact quality with total compression `1.3264248705x`, p95 `15.364934 ms`, and `204234.21` scores/s. Treat these rows as serving proxy evidence tied to release sealed SHA256 `601932251333d6bb5279cbe6c042a7b7f188037a47501f3134071e51baa5f2f7`, not CorkScrewDB API smoke.
...
The EOS-named targeted-v3 package is the measured dense promotion candidate for the `eos-embed-v1` lineage. The durable CorkScrewDB external-provider asset lives at `assets/corkscrewdb-default-embedder/corkscrewdb-default-embedder.mll`; the ignored release run `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/` remains provenance. It was trained from `43` targeted no-teacher rows; release sealed SHA256 is `601932251333d6bb5279cbe6c042a7b7f188037a47501f3134071e51baa5f2f7`, release package SHA256 is `188265db16992ab24be15e678c5f7e175bebad769e8d844e8b0f50ffc23bd5bf`, and tokenizer SHA256 is `64cf63223cb3f97125040677a573e6ab6c625cff1f6f338f4e680a4c9f7a42f5`. Package and sealed inspection report `package verify: OK`, sealed inspection reports `package: embedded sealed MLL`, and final plus hard eval logs record `optimizer_updates=0`. The legacy source artifact `runs/eos-embed-v1-targeted-neargate-v3-low-lr-restorebest-20260614T000000Z/targeted-v3-lr000002-restorebest-manta/manta-embed-v1.sealed.mll`, SHA256 `ea776e2fca7fdade7ee05396b2ee8980e220899e2515853c83a4bca34cf87242`, remains provenance only. The June 10 deephard-full candidate remains the previous strict sealed anchor for comparison: `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z/manta-embed-v1.sealed.mll`, SHA256 `a7461b47784ea7434cf6048f33f6c281ef19887cfa9d0c699b6f2fba079f2b67`, with sealed scoreboard under `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z-sealed-scoreboard/`.

Repeatable release validation is `ferrous-wheel run scripts/smoke_eos_default_embedder_release.fw`; with `EOS_DEFAULT_EMBEDDER_RELEASE_CORKSCREWDB_ROOT=/home/draco/work/corkscrewdb`, it also starts CorkScrewDB without `-eos-artifact` and checks the temp manifest records embedding id `corkscrewdb-default-embedder` dim `256`. The CorkScrewDB provider promotion is checkpointed in commit `d19780622b3e879bedfdc202e84966d6437441d5`.
