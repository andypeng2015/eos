...
For the promoted nf005 compact policy, prefer the repo-local serving proxy:
...
It selects q4/fp16/rerank-overfetch=200 and writes `summary.tsv` plus `manifest.json` under `runs/eos-default-embedder-serving-smoke-<timestamp>/`. This is not an actual CorkScrewDB API smoke; it is the in-repo TurboQuant serving proxy. For the nf005 release artifact, q4/fp16/175 failed direct checked-in FiQA recall against predecessor q4/fp16/250 (`0.349080` versus `0.350444`). q4/fp16/200 is the corrected compact policy and the lowest checked-in-asset recovery-sweep profile that clears predecessor quality across SciFact, NFCorpus, and FiQA: SciFact nDCG delta `+0.002216`, recall delta `0`; NFCorpus nDCG delta `+0.001402`, recall delta `+0.000168`; FiQA nDCG delta `+0.000815`, recall delta `+0.001337`; total compression `1.5900621118x`. Treat these legacy rows as serving proxy selection evidence tied to release sealed SHA256 `8074d2fce1842e232df2b4172d40463d82b57848c719b2d76fdd68aca682ac70`, not CorkScrewDB API smoke and not the formal compact `gate-scoreboard` anchor because they lack `quantizer_seed`. The historical strict FiQA recall reference is `0.3516782086`. The formal seeded q4/fp16/o200 anchor is `runs/eos-nf005-compact-anchor-provenance-repair-v1-20260619T091223Z/anchor-q4-fp16-overfetch200-scoreboard/scoreboard.json`; it uses `quantizer_seed=5581486560434873699` and records FiQA recall `0.3512924062` and NFCorpus recall `0.2436904780`. Future compact-aware candidates should report both the formal seeded gate and the historical strict FiQA reference until a new release policy supersedes it. q4/150 was not selected because later paired evidence showed recall instability.
...
Current measured release-artifact compact-profile result: q4/fp16 sidecar rerank at overfetch200 is the promoted nf005 compact policy. It is selected because q4/175 failed direct checked-in FiQA recall, while q4/200 preserves selected quality across SciFact, NFCorpus, and FiQA against previous q4/fp16/250 with total compression `1.5900621118x`. The legacy recovery-sweep row is historical selection evidence and a stricter FiQA recall reference (`0.3516782086`), while formal future compact gates use the regenerated seeded anchor at `runs/eos-nf005-compact-anchor-provenance-repair-v1-20260619T091223Z/anchor-q4-fp16-overfetch200-scoreboard/scoreboard.json` (`quantizer_seed=5581486560434873699`, FiQA recall `0.3512924062`, NFCorpus recall `0.2436904780`). Do not generalize this result to direct q4, remote CorkScrewDB, HNSW, federation, or hosted-model parity.
...
The nf005 Qwen3-continuation package is the measured dense promotion candidate for the `eos-embed-v1` lineage. The durable CorkScrewDB external-provider asset lives at `assets/corkscrewdb-default-embedder/corkscrewdb-default-embedder.mll`; the ignored release run `runs/current-release-qwen3-nf005-continuation-20260616T224102Z/candidate/` remains provenance. It was trained from `qwen3.filtered.hard-negatives.jsonl` with teacher source weights `scifact=1,nfcorpus=0.05,fiqa=0.25`, LR `0.000002`, and quality target `pairwise`; release sealed SHA256 is `8074d2fce1842e232df2b4172d40463d82b57848c719b2d76fdd68aca682ac70`, release package SHA256 is `188265db16992ab24be15e678c5f7e175bebad769e8d844e8b0f50ffc23bd5bf`, and tokenizer SHA256 is `64cf63223cb3f97125040677a573e6ab6c625cff1f6f338f4e680a4c9f7a42f5`. Package and sealed inspection report `package verify: OK`, sealed inspection reports `package: embedded sealed MLL`, and final plus hard eval logs record `optimizer_updates=0`. The targeted-v3 package remains predecessor default evidence. The June 10 deephard-full candidate remains the previous strict sealed anchor for comparison: `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z/manta-embed-v1.sealed.mll`, SHA256 `a7461b47784ea7434cf6048f33f6c281ef19887cfa9d0c699b6f2fba079f2b67`, with sealed scoreboard under `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z-sealed-scoreboard/`.

Repeatable release validation is `ferrous-wheel run scripts/smoke_eos_default_embedder_release.fw`; with `EOS_DEFAULT_EMBEDDER_RELEASE_CORKSCREWDB_ROOT=/home/draco/work/corkscrewdb`, it also starts CorkScrewDB without `-eos-artifact` and checks the temp manifest records embedding id `corkscrewdb-default-embedder` dim `256`. Current nf005 local flat packed-parent API release-hardening evidence is attached in `.tiller/scratch/codex/eos-nf005-current-default-corkscrewdb-api-smoke-v1-report.md`, with q4 nDCG@10 `0.451122`, recall@100 `0.755222`, DB directory multiple `0.020372x`, vector payload multiple `0.013312x`, p95 `142.971679ms`, planner fit `180`, and target fit `true`; q8 remains diagnostic because target fit is `false`. The CorkScrewDB provider promotion is checkpointed in commit `d19780622b3e879bedfdc202e84966d6437441d5`.
