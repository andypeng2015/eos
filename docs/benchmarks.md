...
The latest reranker-teacher frontier audit generated request rows and now has working true-reranker plumbing, but still does not unblock training. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` contains `1,188` NFCorpus train/dev hard-negative examples and `7,066` scorer requests, all request-only and non-trainable. `cmd/teacher-bridge --mode tei-rerank` works against TEI `/rerank`; on RTX50/Blackwell, `ghcr.io/huggingface/text-embeddings-inference:120-1.9` is the viable local image. The decision is `no_train` / `teacher_quality_blocked`: tested subset64 teachers failed the quality gates (`bge-reranker-base` positive-top1 `0.421875`, kept `27/64`; `BAAI/bge-reranker-v2-m3` `0.140625`, kept `9/64`; `BAAI/bge-reranker-large` `0.234375`, kept `15/64`; `tomaarsen/Qwen3-Reranker-0.6B-seq-cls` `0.078125`, `5/64`, mean margin `-0.5452850256115198`). The Qwen3 seq-cls audit was no-train: TEI `120-1.9` failed with `classifier model type is not supported for Qwen3`, while Python `AutoModelForSequenceClassification` with Qwen3 model-card formatting on CUDA RTX 5070 Ti was mechanically scoreable and preserved request-only metadata after filtering (`cleared=59`, `dropped=0`). The next benchmark action is label/teacher-disagreement investigation or a different teacher, followed by positive-top1/margin/novelty/leakage audits before any dense training run.
