...
Corrected q2-341 packed-parent frontier evidence is now an API run rather than planner math alone. Original corrected wrapper run `runs/eos-corkscrewdb-timeseries-window-q2-341-variants20-packed-minimal-corrected-20260616T000000Z/` generated the child/query/qrels inputs and packed planner evidence for q2 `128d`, `packed_parent_multivectors`, `packed_metadata_mode=none`, and `packed_child_id_mode=ordinal` with `100` parents, `34,100` child windows, and `341` windows per parent. Compact snapshot v5 child-vector smoke rerun `runs/eos-corkscrewdb-q2-341-compact-snapshot-v5-20260616T000000Z/`, against CorkScrewDB commit `c208f9b50d29f9fdf19771c4b093332c7c8fd0b4` (`update(snapshot): Add compact format for quantized ordinal children`), measured the improved persisted DB directory on those inputs. It recorded `quantized_vector_bytes=36`, `quantized_child_bytes=1,227,600`, vector payload multiple `0.9990234375x`, packed planner overhead `32`, packed planner bytes `12,308`, packed planner multiple `0.999025974025974x`, measured DB directory bytes `1,237,808`, DB directory multiple `1.0073307291666667x`, `ndcg@10=0.4493940305106442`, `recall@100=1.000000`, and p95 `1.521388ms`. With CorkScrewDB compact snapshot v5 ordinal encoding, the persisted DB directory is approximately one dense parent-vector budget for this strict shape; without that compact snapshot path, or for richer child records, keep DB directory cost separate.
