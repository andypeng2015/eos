...
The latest reranker-teacher frontier audit generated request rows and now has working true-reranker plumbing, but still does not unblock training. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` contains `1,188` NFCorpus train/dev hard-negative examples and `7,066` scorer requests, all request-only and non-trainable. `cmd/teacher-bridge --mode tei-rerank` works against TEI `/rerank`; on RTX50/Blackwell, `ghcr.io/huggingface/text-embeddings-inference:120-1.9` is the viable local image. The decision is `no_train` / `teacher_quality_blocked`: cleaned subset64 BGE teachers still failed the quality gates (`BAAI/bge-reranker-base` selected/any-positive top1 `29/64` or `0.453125`, kept `29/64`; `BAAI/bge-reranker-large` `22/64` or `0.343750`, kept `22/64`; `BAAI/bge-reranker-v2-m3` `10/64` or `0.156250`, kept `10/64`). The cleaned BGE audits report `duplicate_positive_negative_candidates=0`; dirty-to-cleaned selected-top1 changed by base `+2`, large `+7`, and v2-m3 `+1`. Use `audit-teacher-scores --qrels <qrels.tsv> --corpus <corpus.jsonl>` when provenance exists to add duplicate/multi-positive-aware diagnostics. The Qwen3 seq-cls audit remains historical dirty-subset no-train evidence, not a cleaned rerun: TEI `120-1.9` failed with `classifier model type is not supported for Qwen3`, while Python `AutoModelForSequenceClassification` with Qwen3 model-card formatting on CUDA RTX 5070 Ti was mechanically scoreable and preserved request-only metadata after filtering (`positive-top1=0.078125`, kept `5/64`, cleared `59`, dropped `0`, mean margin `-0.5452850256115198`). The next benchmark action is label/teacher-disagreement investigation or a different teacher, followed by positive-top1/margin/novelty/leakage audits before any dense training run.
