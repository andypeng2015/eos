...
Repeatable release validation is `ferrous-wheel run scripts/smoke_eos_default_embedder_release.fw`; with `EOS_DEFAULT_EMBEDDER_RELEASE_CORKSCREWDB_ROOT=/home/draco/work/corkscrewdb`, it also starts CorkScrewDB without `-eos-artifact` and checks the temp manifest records embedding id `corkscrewdb-default-embedder` dim `256`. Current local flat packed-parent API release-hardening evidence remains predecessor nf005 context in `.tiller/scratch/codex/eos-nf005-current-default-corkscrewdb-api-smoke-v1-report.md`, with q4 nDCG@10 `0.451122`, recall@100 `0.755222`, DB directory multiple `0.020372x`, vector payload multiple `0.013312x`, p95 `142.971679ms`, planner fit `180`, and target fit `true`; q8 remains diagnostic because target fit is `false`. The CorkScrewDB provider promotion is checkpointed in commit `d19780622b3e879bedfdc202e84966d6437441d5`.
