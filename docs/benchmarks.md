...
The latest reranker-teacher frontier audit generated request rows and now has working true-reranker plumbing, but still does not unblock training. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` contains `1,188` NFCorpus train/dev hard-negative examples and `7,066` scorer requests, all request-only and non-trainable. `cmd/teacher-bridge --mode tei-rerank` works against TEI `/rerank`; on RTX50/Blackwell, `ghcr.io/huggingface/text-embeddings-inference:120-1.9` is the viable local image. The newer broader selected-diverse BGE audit in `runs/eos-nfcorpus-broader-cleaned-mining-quality-signal-v1-20260623T101157Z/` is the current BGE evidence, not a training gate: `BAAI/bge-reranker-base` selected/any-positive top1 `59/128` (`0.460938`), kept `59/128`; `BAAI/bge-reranker-large` `65/128` (`0.507812`), kept `65/128`; and `BAAI/bge-reranker-v2-m3` `59/128` (`0.460938`), kept `59/128`, all with `duplicate_positive_negative_candidates=0`. BGE large is the strongest BGE signal, but the decision remains `no_train` / `teacher_quality_blocked`: all-three agreement is only `42/128`, at-least-two agreement is `62/128`, all-three is too small/correlated, and at-least-two margins are too weak. The completed non-BGE MiniLM control also stays below the gate: `cross-encoder/ms-marco-MiniLM-L6-v2` selected/any-positive top1 `54/128` (`0.421875`), kept `54/128`, duplicate-positive negative candidates `0`, selected/any mean margin `0.140112`, mean normalized entropy `0.985032`, and filter mean margin after `0.452819`. MiniLM is weaker than BGE-large `65/128` and overlaps heavily with it (`48` kept intersection, top1 candidate agreement `81/128`), so the decision remains `no_train` / `teacher_quality_blocked`. Jina v2 is no longer only a TEI compatibility blocker: local Hugging Face fallback scoring on the 128-row selected-diverse slice produced selected/any-positive top1 `67/128` (`0.523438`), selected mean margin `0.366817`, mean normalized entropy `0.813294`, and kept `67/128`, slightly above BGE-large on that slice; this small-slice result does not generalize to the full frontier. Full-frontier Jina HF scored `7066/7066` request rows and imported `1188/1188` examples after a score/import-dedup sidecar for 8 duplicate `(source, query, candidate)` keys. The full result is `158/1188` selected-positive top1 (`0.132997`), mean margin `-1.110194`, mean normalized entropy `0.873741`, filter kept `158/1188`, with train split `108/932` (`0.115880`) and dev split `50/256` (`0.195312`), so Jina full frontier is `teacher_quality_blocked` / `no_train` and no guarded trainability descriptor should be proposed. Doc-id provenance caveat: qrels any-positive diagnostics could not materialize on the old full frontier because examples lack `query_id`, `positive_doc_id`, and `negative_doc_ids`; duplicate selected-positive-as-negative rows also exist in the full frontier request file, and import-dedup only made the old artifact importable. `mixedbread-ai/mxbai-rerank-base-v2` plus `mixedbread-ai/mxbai-rerank-large-v2` failed TEI `120-1.9` serving even with `--pooling cls` / `--pooling mean` because `sentence_bert_config.json.max_seq_length` is missing; this is a TEI compatibility blocker, not model-quality evidence. Cleaned subset64 metrics are historical initial cleaned evidence (`29/64`, `22/64`, `10/64` for base/large/v2-m3) and the Qwen3 seq-cls audit remains historical dirty-subset no-train evidence, not a cleaned rerun. Generated rows remain `request_only=true` plus `train_allowed=false`. The next benchmark action is to rebuild/remine the full NFCorpus reranker frontier using the cleaned duplicate-positive filters and doc-id provenance now available, then re-audit any teacher; alternatives are synthetic/non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka signal, or larger-model/bootstrap. Do not train from the old full-frontier artifacts.
