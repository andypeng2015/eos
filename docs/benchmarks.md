...
It compares q4/fp16/overfetch250 with q8/fp16/overfetch125 and writes `summary.tsv` plus `manifest.json` under `runs/eos-default-embedder-serving-smoke-<timestamp>/`. This is not an actual CorkScrewDB API smoke; it is the in-repo TurboQuant serving proxy. For targeted-v3, both refreshed q4/fp16 overfetch250 and q8/fp16 overfetch125 sidecar profiles fail strict zero-regression by the same NFCorpus recall@100 miss (`-0.000236936880015165`). Under the explicit `recall@100` tolerance `0.00025`, q4/fp16 overfetch250 is the compact release-candidate profile; q8/fp16 overfetch125 is fallback because compression is lower (`1.326425x` versus q4 `1.590062x`) and the miss shape is identical. Full uncapped SciFact serving proxy run `runs/targeted-v3-default-embedder-serving-smoke-scifact-full-20260616T000000Z/` measured q4/fp16/250 at nDCG@10 `0.562322`, recall@100 `0.796444`, total compression `1.590062x`, p50/p95/p99/max `6.886103/7.656733/8.063955/8.296993 ms`, `411916.62` scores/s, and `75,000` rerank scores; q8/fp16/125 matched the same SciFact quality with total compression `1.326425x`, p95 `13.127147 ms`, and `214167.55` scores/s. Treat these rows as serving proxy evidence, not CorkScrewDB API smoke.
...
Current measured targeted-v3 compact-profile result: q4/fp16 sidecar rerank at overfetch250 and q8/fp16 sidecar rerank at overfetch125 were refreshed in `runs/targeted-v3-compact-rerank-refresh-20260616T000000Z/` and checked with dense-anchor tolerance gates in `runs/targeted-v3-compact-tolerance-gate-20260616T000000Z/`. q4/fp16 overfetch250 keeps total compression `1.590062x`; q8/fp16 overfetch125 keeps total compression `1.326425x`. Both preserve dense nDCG@10 on SciFact, NFCorpus, and FiQA, and both preserve recall@100 on SciFact and FiQA, but both fail strict zero-regression promotion on NFCorpus recall@100 by `-0.000236936880015165`. The miss is explained by exactly two dense rank-100 relevant docs moving to fp16 rerank rank 101: `PLAIN-1288` / `MED-2448` and `PLAIN-2680` / `MED-4195`. With the explicit `recall@100` tolerance `0.00025`, q4/fp16 overfetch250 passes all 9 selected checks and is the compact RC; q8/fp16 overfetch125 also passes but remains fallback because compression is lower and the miss shape is identical. Direct q8 diagnostics are non-regressing in the measured direct metrics, but direct rows are not the default-promotion rows. The q4 dense-sidecar control fixes NFCorpus but fails FiQA recall by `-0.0005144032921810537` and compresses only `0.885813x`, so it is not a deployable compact default. Do not describe the compact alias as strict zero-regression promotion.
