...
The latest reranker-teacher frontier audits generated request rows and working true-reranker plumbing, but still do not unblock training. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` contains the older request-only NFCorpus train/dev frontier; it proved TEI `/rerank` plumbing with `cmd/teacher-bridge --mode tei-rerank`, and on RTX50/Blackwell the viable local image is `ghcr.io/huggingface/text-embeddings-inference:120-1.9`. The newer broader selected-diverse BGE audit in `runs/eos-nfcorpus-broader-cleaned-mining-quality-signal-v1-20260623T101157Z/` is audit evidence, not a training gate: `BAAI/bge-reranker-base` selected/any-positive top1 `59/128` (`0.460938`), `BAAI/bge-reranker-large` `65/128` (`0.507812`), and `BAAI/bge-reranker-v2-m3` `59/128` (`0.460938`), all with `duplicate_positive_negative_candidates=0`. BGE large is the strongest BGE signal on that slice, but all-three agreement kept only `42/128`, at-least-two agreement kept `62/128`, and margins were too weak. The MiniLM control stayed weaker than BGE-large at `54/128` (`0.421875`) with selected/any mean margin `0.140112`.

The decisive Jina evidence is the cleaned doc-id-provenance full-frontier audit at `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/`, summarized in `.tiller/scratch/codex/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-report.md`. It rebuilt the frontier to `1188` selected examples (`932` train + `256` dev), exported and scored `7128/7128` request rows, imported `1188/1188` examples with missing `0`, and passed clean validation: doc-id provenance present, duplicate qrels/text positive negatives `0`, duplicate `(source, query, candidate)` request keys `0`, and selected unscored `teacher_scores` rows `0`. Jina selected/any-positive top1 was only `160/1188 = 0.134680`, mean margin `-1.182826`, and mean normalized entropy `0.883789`; the filter kept `160`, cleared `1028`, and dropped `0`. Train split top1 was `122/932 = 0.130901`; dev split was `38/256 = 0.148438`. Decision: `teacher_quality_blocked` / `no_train`; do not train from these rows. The earlier 128-row Jina selected-diverse slice (`67/128 = 0.523438`) was an optimistic small-slice result, and the old dirty full frontier was not sufficient provenance. After the cleaned rebuild, Jina still fails teacher-quality gates. `mixedbread-ai/mxbai-rerank-base-v2` plus `mixedbread-ai/mxbai-rerank-large-v2` remain TEI compatibility blockers because `sentence_bert_config.json.max_seq_length` is missing, not model-quality evidence. Next benchmark action should pivot away from Jina toward BGE-large/mxbai compatibility, synthetic or non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka objectives, or larger-model/bootstrap lanes.
