...
Model-hard and BM25 mining can now emit `teacher_scores` for each query's positive plus explicit negatives. Use `manta export-teacher-score-requests <hard-negatives.jsonl> <requests.jsonl>` to hand the same candidate groups to an external scorer, then import scored rows with `manta import-teacher-scores`. Set `MANTA_ALIGN_CANDIDATE_TEACHER_LOSS_WEIGHT` above zero to blend a soft teacher-distribution cross-entropy into hard-negative training, and tune `MANTA_ALIGN_CANDIDATE_TEACHER_TEMPERATURE` when the teacher ranking is too sharp or too flat.
