...
- MS MARCO conflict adjudication is staged but not scored. Zero-margin edge inspection found `5` rows: `3` normalized-identical text ties and `2` distinct-text scorer ties, with no duplicate doc IDs, no qrels-positive leaks, and exact JSON float ties; keep them as regression fixtures, not forced labels. A deterministic stratified `512`-row reranker slice and `2,048` request rows include all five fixtures and preserve legal gates false/false/true. No official offline `Qwen/Qwen3-Reranker-*` model was cached, so no labels were produced; only `Qwen/Qwen3-Embedding-0.6B` and `mixedbread-ai/mxbai-embed-large-v1` were available, and the known-bad seq-cls Qwen3 reranker path was not used.
...
3. Continue model-improvement work only with a changed signal or objective: for MS MARCO hard-candidate research, first make an official Qwen3 reranker available offline and score the prepared `512`-row slice, or build a score-spectrum artifact only after adjudication. Do not spend more on strict clean-agreement LR tuning; otherwise continue larger-model bootstrap work and Matryoshka-style dimension slicing rather than treating BM25-dot hybrid retrieval rows as dense model improvement.
