...
3. Continue model-improvement work only with a changed signal or objective: for MS MARCO hard-candidate research, change one LR or teacher/loss knob and require matched pairwise/hard lift before retrieval scoreboard spend; otherwise continue larger-model bootstrap work and Matryoshka-style dimension slicing rather than treating BM25-dot hybrid retrieval rows as dense model improvement.
