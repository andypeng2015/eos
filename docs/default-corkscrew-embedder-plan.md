...
Use `--tolerance` only for an explicitly accepted numeric rounding margin. For TurboQuant rows, add the matching `--baseline`, `--method`, and `--bits` filters so the command compares one unambiguous row per dataset in both scoreboards. The release-artifact compact gate covers q4/fp16 overfetch250 only under the explicit `recall@100` tolerance `0.00025`; it is not a strict zero-regression promotion. Strict zero-regression compact promotion still fails: q4/fp16/250 matches dense nDCG@10 on SciFact, NFCorpus, and FiQA, but misses NFCorpus recall@100 by `-0.000236936880015165`. The accepted compact policy is therefore explicit: q4/fp16 overfetch250 is the compact release-candidate profile only under that tolerance. The release-artifact full SciFact serving proxy measured q4/fp16/250 at nDCG@10 `0.5623217863`, recall@100 `0.7964444444`, p95 `7.607784ms`, `414712.08` scores/s, and total compression `1.5900621118x`; q8/fp16/125 is a lower-compression measured serving fallback, not the compact RC or compact-gate claim, at p95 `15.364934ms`, `204234.21` scores/s, and compression `1.3264248705x`.
...
The smoke compares `turboquant_ip_b4_overfetch250_fp16_rerank` against the lower-compression fallback `turboquant_ip_b8_overfetch125_fp16_rerank`, writes a summary TSV and manifest under `runs/eos-default-embedder-serving-smoke-<timestamp>/`, and can gate total compression plus optional p95 latency. This is CorkScrewDB-relevant TurboQuant serving proxy evidence only, not a CorkScrewDB API load/index/search smoke. The full uncapped SciFact release-artifact run `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/serving-smoke-scifact-full/` used release sealed SHA256 `601932251333d6bb5279cbe6c042a7b7f188037a47501f3134071e51baa5f2f7` and measured q4/fp16/250 at nDCG@10 `0.5623217863`, recall@100 `0.7964444444`, total compression `1.5900621118x`, p95 `7.607784 ms`, `414712.08` scores/s, and `75,000` rerank scores. The q8/fp16/125 fallback matched the same SciFact quality, with total compression `1.3264248705x`, p95 `15.364934 ms`, and `204234.21` scores/s. Use the local flat CorkScrewDB API smoke when the actual `PutVector`/`SearchVector` integration path needs to pass:
...
Current release-artifact local Eos TurboQuant result: q4/fp16 sidecar rerank at overfetch250 was refreshed in `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/compact-q4-fp16-rerank250-scoreboard/`, then checked against dense-anchor scoreboards in `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/compact-q4-fp16-rerank250-gate/`. q4/fp16 overfetch250 has total compression `1.590062x`. It preserves dense nDCG@10 on SciFact, NFCorpus, and FiQA, and preserves recall@100 on SciFact and FiQA, but fails strict zero-regression promotion on NFCorpus recall@100 by `-0.000236936880015165`. With the explicit `recall@100` tolerance `0.00025`, q4/fp16 overfetch250 passes all 9 checks across SciFact, NFCorpus, and FiQA for nDCG@10, recall@100, and total compression. Do not describe the compact alias as strict zero-regression promotion.
...
- A CorkScrewDB load/index/search smoke has passed with the candidate vectors; use `scripts/smoke_corkscrewdb_child_vectors.fw` for the local flat API path and override its input-cache env vars for candidate-specific child vectors. Keep `EOS_CORKSCREW_SMOKE_LAYOUT=separate_child_vectors` for the current child-overfetch baseline, or set `EOS_CORKSCREW_SMOKE_LAYOUT=packed_parent_multivectors` to measure local flat `PutMultiVector` plus exact `SearchParentsVector` packed-parent persistence. Current release-artifact packed-parent evidence uses fresh cache `runs/eos-vector-caches/eos-release-targeted-v3-scifact-child-w128-o32-128d/` whose manifest points at the EOS release sealed artifact. The local flat q4 row records nDCG@10 `0.451009`, recall@100 `0.771889`, DB multiple `0.020372x`, p95 `10.790243ms`, planner fit `180`, and target fit `true`; q8 is diagnostic at nDCG@10 `0.469385`, recall@100 `0.780222`, DB multiple `0.032901x`, p95 `19.877473ms`, planner fit `93`, and target fit `false`.
