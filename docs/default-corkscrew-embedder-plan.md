...
The corrected q2 packed-parent frontier has also been measured through the same local flat API path after TurboQuant IP byte-accounting and packed overhead fixes. Verified run `runs/eos-corkscrewdb-timeseries-window-q2-341-variants20-packed-minimal-corrected-20260616T000000Z/` used q2 `128d`, `packed_parent_multivectors`, `packed_metadata_mode=none`, and `packed_child_id_mode=ordinal` for `100` parents and `34,100` child windows, `341` windows per parent. It recorded `quantized_vector_bytes=36`, `quantized_child_bytes=1,227,600`, vector payload multiple `0.9990234375x`, packed planner overhead `32`, packed planner bytes `12,308`, packed planner multiple `0.999025974025974x`, measured DB directory bytes `2,453,118`, DB directory multiple `1.9963525390625x`, `ndcg@10=0.449394`, `recall@100=1.000000`, and p95 `1.487102ms`. Cite this as local flat packed-parent API evidence for the near-one vector payload and packed planner budget at the q2-341 frontier, not as proof that the full persisted DB directory fits under one dense parent-vector budget.
