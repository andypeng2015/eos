...
Use `--tolerance` only for an explicitly accepted numeric rounding margin. For TurboQuant rows, add the matching `--baseline`, `--method`, and `--bits` filters so the command compares one unambiguous row per dataset in both scoreboards. The targeted-v3 compact-profile refresh is complete for q4/fp16 overfetch250 and q8/fp16 overfetch125. Strict zero-regression compact promotion still fails: both rows match dense nDCG@10 on SciFact, NFCorpus, and FiQA, but both miss NFCorpus recall@100 by `-0.000236936880015165` because two dense rank-100 relevant docs become fp16 rerank rank 101. The accepted compact policy is therefore explicit: q4/fp16 overfetch250 is the targeted-v3 compact release-candidate profile only under a `recall@100` tolerance of `0.00025`; it is not a strict zero-regression promotion.
...
The smoke compares `turboquant_ip_b4_overfetch250_fp16_rerank` against the lower-compression fallback `turboquant_ip_b8_overfetch125_fp16_rerank`, writes a summary TSV and manifest under `runs/eos-default-embedder-serving-smoke-<timestamp>/`, and can gate total compression plus optional p95 latency. This is CorkScrewDB-relevant TurboQuant serving proxy evidence only, not a CorkScrewDB API load/index/search smoke. The full uncapped SciFact targeted-v3 run `runs/targeted-v3-default-embedder-serving-smoke-scifact-full-20260616T000000Z/` used the active targeted-v3 artifact and measured q4/fp16/250 at nDCG@10 `0.562322`, recall@100 `0.796444`, total compression `1.590062x`, p50/p95/p99/max `6.886103/7.656733/8.063955/8.296993 ms`, `411916.62` scores/s, and `75,000` rerank scores. The q8/fp16/125 fallback matched the same SciFact quality, with total compression `1.326425x`, p95 `13.127147 ms`, and `214167.55` scores/s. Use the local flat CorkScrewDB API smoke when the actual `PutVector`/`SearchVector` integration path needs to pass:
...
Current targeted-v3 local Eos TurboQuant result: q4/fp16 sidecar rerank at overfetch250 and q8/fp16 sidecar rerank at overfetch125 were refreshed in `runs/targeted-v3-compact-rerank-refresh-20260616T000000Z/`, then checked against dense-anchor scoreboards in `runs/targeted-v3-compact-tolerance-gate-20260616T000000Z/`. q4/fp16 overfetch250 has total compression `1.590062x`; q8/fp16 overfetch125 has total compression `1.326425x`. Both preserve dense nDCG@10 on SciFact, NFCorpus, and FiQA, and both preserve recall@100 on SciFact and FiQA, but both fail strict zero-regression promotion on NFCorpus recall@100 by `-0.000236936880015165`. The diagnosis is a known fp16 sidecar boundary effect: exactly `PLAIN-1288` / `MED-2448` and `PLAIN-2680` / `MED-4195` move from dense rank 100 to fp16 rerank rank 101. With the explicit `recall@100` tolerance `0.00025`, q4/fp16 overfetch250 passes all 9 checks across SciFact, NFCorpus, and FiQA for nDCG@10, recall@100, and total compression. q8/fp16 overfetch125 also passes under that tolerance, but q4 is preferred because compression is higher (`1.590062x` versus `1.326425x`) and the miss shape is identical. Direct q8 rows are useful diagnostics and non-regressing in the measured direct metrics, but direct rows are not the default-promotion rows. The q4 dense-sidecar control fixes NFCorpus but fails FiQA recall by `-0.0005144032921810537` and compresses only `0.885813x`, so it is not a deployable compact default.
...
1. Build the dense plus compact targeted-v3 release package and alias audit: dense remains the strict zero-regression default candidate, while q4/fp16 overfetch250 is the compact RC only under the explicit `recall@100` tolerance `0.00025`.
2. Finish the final release manifest, SHA256/checksum packaging, and `corkscrewdb-default-embedder` alias audit without implying strict zero-regression compact promotion.
