...
Use `--tolerance` only for an explicitly accepted numeric rounding margin. For TurboQuant rows, add the matching `--baseline`, `--method`, and `--bits` filters so the command compares one unambiguous row per dataset in both scoreboards. The targeted-v3 compact-profile refresh is complete for q4/fp16 overfetch250 and q8/fp16 overfetch125, and neither row is eligible for alias promotion under the strict zero-regression gate: both match dense nDCG@10 on SciFact, NFCorpus, and FiQA, but both miss NFCorpus recall@100 by `-0.000236936880015165` because two dense rank-100 relevant docs become fp16 rerank rank 101. Gate any compact alias change only after an explicit tolerance/exception policy or a new compact profile passes.
...
1. Decide the compact promotion policy: either accept an explicit NFCorpus fp16 boundary tolerance for q4/fp16 overfetch250 or require a new compact profile that passes zero-regression without the dense-sidecar storage loss.
2. Run serving proxy or alias work only after that compact tolerance is accepted or a new compact profile passes; keep the targeted-v3 CorkScrewDB packed-parent smoke separate from q4/fp16 alias promotion.
3. If shipping dense first, finish the dense RC package, manifest, SHA256, and alias audit for `corkscrewdb-default-embedder` without claiming compact default promotion.
