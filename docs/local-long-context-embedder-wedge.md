...
- Teacher temperature is now an active SOTA knob. Holding `teacher_loss_weight=0.20`, `teacher_temperature=0.75` passed the gate at macro `0.147738` but stayed below the temperature-`1.0` run. Softer `teacher_temperature=1.5` is the new retrieval best at macro `0.148143`: SciFact `+0.006702`, NFCorpus `-0.000231`, and FiQA `+0.001256` versus the fresh-mined baseline, with recall floors intact.
