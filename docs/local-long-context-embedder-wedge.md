...
Before a real LongEmbed-style dataset is wired into the scoreboard, `scripts/smoke_eos_long_context_chunked_baseline.fw` provides a reproducible synthetic late-needle smoke for the long-document comparison shape. The current promoted-default rerun is `runs/eos-current-default-long-context-chunked-baseline-v1-20260623T055729Z/`, summarized in `.tiller/scratch/codex/eos-current-default-long-context-wedge-rerun-v1-report.md`: direct Eos nDCG@10 `0.119502020`, single-vector cache nDCG@10 `0.157617787`, and chunked dense/q2/q4/q8 child-max rows at nDCG@10 and recall@100 `1.0`. This remains synthetic smoke only, not LongEmbed proof or a real long-document benchmark; BM25 also reaches a perfect score on the generated dataset.
...
For short-set parent/single-vector compact-cache evidence, `scripts/eval_eos_prefix_dim_curve.fw` records the s40-era default artifact's prefix-width curve separately from long-document child-vector evidence. Its rows use `export-retrieval-vectors --output-dim` prefix truncation plus L2 renormalization, not trained Matryoshka/native 128d heads. The full-query `128d` q4/q8 run at `runs/eos-prefix-128-q4q8-full-query-current-default-v1-20260620T021328Z/` keeps `128d q8` as the predecessor near-dense compact cache reference at about `3.878788x` vector-payload compression, while `128d q4` is the predecessor aggressive compact candidate at about `7.529412x`. q2 is not default-quality-ready without a trained compact head. Keep this lane distinct from repo-docs child-vector and token-span evidence, which exercises multi-vector rollups and long-document retrieval shapes, and refresh it before using it as new-default evidence.
...
Both best rows beat Eos default repo-docs dense `0.644872` on this local lane, while still trailing Qwen3 0.6B 128d child q4 `0.739269` and mxbai-large 128d child q4 `0.691586`. Reports and ignored run roots: `runs/eos-lc-token-span-sweep-v1-20260619T004023Z`, `runs/eos-lc-compact-token-span-sweep-v1-20260619T005356Z`, `.tiller/scratch/codex/eos-lc-token-span-sweep-v1-report.md`, and `.tiller/scratch/codex/eos-lc-compact-token-span-sweep-v1-report.md`. The current promoted-default capped cache comparison in `runs/eos-current-default-long-context-wedge-cache-v1-20260623T055955Z/` used the local repo-docs dataset and matching local Qwen3/mxbai caches only. All rows remain `quality_claim=false`: Eos direct nDCG@10 `0.634327008`, Eos token-span dense/q4 `0.656691634`/`0.639676979`, best Eos fusion `0.638514924`, Qwen3 dense/q4 `0.807649216`/`0.735872282`, and mxbai dense/q4 `0.783370960`/`0.750464207`; recall@100 saturated at `1.0` for every row. Preserve `quality_claim=false`: repo-docs is tiny deterministic path/heading qrels cache evidence, not LongEmbed proof, and the successful capped run did not prove long-token consumption beyond the observed tokenizer-output cap.
...
The promoted narrow default package has capped official comparison rows against cached Qwen3 0.6B and mxbai-large child-vector baselines. The earlier external blocker is cleared for the official `qmsum` and `2wikimqa` doc20/query20 slices, but these rows are still diagnostic: `quality_claim=false`, capped official slices, cache-mode external vectors, external baselines are off for the local wedge gate, and external rows use many more child vectors and higher storage than Eos token-span rows.

| Dataset | Best Eos row | Eos nDCG@10 | Qwen3 q4 nDCG@10 | mxbai q4 nDCG@10 | Gap vs best external | Caveat |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| `qmsum` | v2 external-only fusion | 0.536813696 | 0.876293470 | 0.806946535 | -0.339480 | official doc20/query20 real-doc capped slice with cache-mode external rows |
| `2wikimqa` | v2 external-only fusion | 0.744085103 | 1.000000000 | 1.000000000 | -0.255915 | official doc20/query20 real-doc capped slice with cache-mode external rows |
| `passkey4096` | fusion | 0.605428 | 0.477549 | 0.357039 | +0.127878 | synthetic fullslice task; diagnostic only |

The current official capped result is negative for the real-doc product claim. In the v2 external-only cache-mode comparison, Eos best macro nDCG@10 over `qmsum` + `2wikimqa` is `0.640449400`, while Qwen3 q4 is `0.938146735` and mxbai q4 is `0.903473268`; Eos-best minus best external is `-0.297697335`. The sparse-enabled rerun keeps the external rows but changes the best Eos fusion values slightly (`qmsum` `0.538700670`, `2wikimqa` `0.739699216`) and adds sparse-encoder parent rows. Those sparse encoder rows are subquadratic under the host-reference audit (`route_block_size=128`, `route_top_blocks=8`, score fraction `0.2578125`) but quality-negative versus direct retrieval on both datasets: `qmsum` sparse dense/q4 `0.458166919`/`0.420963072` versus direct `0.517955842`, and `2wikimqa` sparse dense/q4 `0.504805535`/`0.505901435` versus direct `0.739153782`. The synthetic `passkey4096` Eos fusion win of `+0.127878` is useful for diagnosis but must not be described as product-quality proof. Before any product-quality long-context claim, either change the long-context model/objective or add stronger comparable real-doc Eos rows that close this gap.
