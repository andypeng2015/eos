...
For promotion runs, set `MANTA_ALIGN_GATE_CANDIDATE=1`. The gate writes the summary first, then fails the run if macro nDCG@10 misses `MANTA_ALIGN_MIN_MACRO_NDCG_DELTA` or any dataset regresses more than `MANTA_ALIGN_MAX_DATASET_NDCG_REGRESSION`; `MANTA_ALIGN_MIN_DATASET_NDCG_RATIO` adds an optional per-dataset ratio floor.
