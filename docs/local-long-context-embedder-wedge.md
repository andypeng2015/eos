...
- `scripts/bench_sparse_attention.fw` now archives that preflight beside CUDA sparse-attention benchmark JSONL/text and a parsed summary TSV, so exact f16 and routed TurboQuant sparse kernels can be compared from a clean run directory with selected-key, candidate-budget, score-fraction, subquadratic, and K/V compression metrics attached.
