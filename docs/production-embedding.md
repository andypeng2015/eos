...
Default CorkScrewDB embedder promotion requires a vector-index cost check in addition to dense retrieval quality. The repo-local serving proxy is `scripts/smoke_eos_default_embedder_serving.fw`; for the promoted nf005 compact policy, it selects the q4/fp16/rerank-overfetch=200 profile and records p50/p95/p99/max per-query scoring latency:
...
This smoke uses the in-repo TurboQuant evaluator as CorkScrewDB-relevant vector-index and serving proxy evidence. It is not an actual CorkScrewDB API load/index/search smoke. For the promoted nf005 release artifact, q4/fp16 overfetch175 failed direct checked-in FiQA recall versus predecessor q4/fp16/250 (`0.349080` versus `0.350444`). The corrected compact policy is q4/fp16 overfetch200: it was selected from historical checked-in-asset recovery-sweep evidence as the lowest profile that clears predecessor q4/fp16/250 quality across SciFact, NFCorpus, and FiQA, with total compression `1.5900621118x`. Compact promotion anchors must be regenerated with the current deterministic TurboQuant evaluator and the selected scoreboard rows must include matching `quantizer_seed`; pre-`566e309` random-seeded scoreboards are historical evidence only and must not be used as compact promotion anchors. The historical q4/200 explicit comparison recorded SciFact nDCG delta `+0.002216` and recall delta `0`, NFCorpus nDCG delta `+0.001402` and recall delta `+0.000168`, and FiQA nDCG delta `+0.000815` and recall delta `+0.001337`, with strict historical FiQA recall `0.3516782086`. The formal reproducible q4/fp16/o200 compact gate anchor is `runs/eos-nf005-compact-anchor-provenance-repair-v1-20260619T091223Z/anchor-q4-fp16-overfetch200-scoreboard/scoreboard.json`; its selected rows carry `quantizer_seed=5581486560434873699` and record NFCorpus recall `0.2436904780` and FiQA recall `0.3512924062`. Future compact-aware candidates should report both the formal seeded gate result and the historical strict FiQA reference `0.3516782086` until a new release policy supersedes this split. The compact-aware prepared-IP q4 rerun is no-promote because it matches the seeded compact behavior but does not improve over nf005 and does not clear the historical stricter FiQA reference. The q4/150 row was not selected because later paired evidence showed recall instability. The promoted row is tied to release sealed SHA256 `8074d2fce1842e232df2b4172d40463d82b57848c719b2d76fdd68aca682ac70`. For the local flat CorkScrewDB API path, run `scripts/smoke_corkscrewdb_child_vectors.fw`; by default it creates or consumes child-vector caches, loads child vectors with CorkScrewDB `PutVector`, queries with `SearchVector`, rolls child hits up to parent IDs by max score, and records storage, latency, nDCG@10, and recall@100:
...
Current nf005 packed-parent local flat API evidence is attached to the checked-in default in `.tiller/scratch/codex/eos-nf005-current-default-corkscrewdb-api-smoke-v1-report.md`, with run `runs/eos-nf005-current-default-corkscrewdb-budget-quality-packed-q4q8-20260617T142823Z/` and vector cache `runs/eos-vector-caches/eos-nf005-current-default-scifact-child-w128-o32-128d/`. The q4 release-gate row used `packed_parent_multivectors`, `packed_metadata_mode=none`, and ordinal child IDs, recording nDCG@10 `0.451122`, recall@100 `0.755222`, DB directory multiple `0.020372x`, vector payload multiple `0.013312x`, p95 `142.971679ms`, planner fit `180`, `target_child_fit=true`, and target storage multiple `0.554545x`. The q8 row is diagnostic only: nDCG@10 `0.472867`, recall@100 `0.780222`, DB directory multiple `0.032901x`, p95 `22.988553ms`, planner fit `93`, and `target_child_fit=false`. This direct quantized packed-parent API evidence stays separate from the q4/fp16/rerank-overfetch=200 serving proxy evidence, and the q4 p95 is run evidence rather than a service SLO.

For the separate parent/single-vector compact-cache lane, use `scripts/eval_eos_prefix_dim_curve.fw`. It exports prefix-dimension document/query caches with `export-retrieval-vectors --output-dim`, L2-renormalizes those prefix vectors, and scores dense plus direct TurboQuant q-bit vector indexes. The full-query current-default `128d` q4/q8 run `runs/eos-prefix-128-q4q8-full-query-current-default-v1-20260620T021328Z/` records `128d q8` as the near-dense compact cache reference at `0.257812` storage multiple, or about `3.878788x` vector-payload compression: SciFact `0.492185`/`0.760778`, NFCorpus `0.184394`/`0.215245`, and FiQA `0.097320`/`0.301407` nDCG@10/recall@100. `128d q4` is the aggressive compact candidate at `0.132812` storage multiple, or about `7.529412x` vector-payload compression: SciFact `0.478427`/`0.758556`, NFCorpus `0.178007`/`0.216910`, and FiQA `0.092377`/`0.297274`. These metrics are vector-payload accounting, not full CorkScrewDB DB-byte evidence, not q4/fp16 rerank evidence, and not proof of a trained Matryoshka/native 128d head. q2 remains out of the default-quality lane without a trained compact head.
...
The default run feeds the Eos 128d SciFact child cache and qrels into `scripts/smoke_corkscrewdb_child_vectors.fw` with q4 overfetch `100,12468`, then joins those `PutVector`/`SearchVector` metrics to `plan-multivector-storage` with `--dim 128 --baseline-dim 3072 --vector-overhead-bytes 32 --sidecar-storage none`. It writes a combined `summary.tsv`, `manifest.json`, command logs, nested `corkscrewdb/` run artifacts, and `planner.json` under `runs/eos-corkscrewdb-budget-quality-smoke-<timestamp>/`. Current default API evidence: q4 overfetch100 has `ndcg@10=0.407586`, `recall@100=0.724111`, DB directory multiple `0.048935x`, and p95 search latency about `11.8ms`; q4 full overfetch has `recall@100=0.741889`; the planner fit is `123` q4 children, so the 100-child target fits. The packed parent variant can now pass through `EOS_CORKSCREW_BUDGET_QUALITY_PACKED_METADATA_MODE` and `EOS_CORKSCREW_BUDGET_QUALITY_PACKED_CHILD_ID_MODE`. Current nf005 packed-parent API run `runs/eos-nf005-current-default-corkscrewdb-budget-quality-packed-q4q8-20260617T142823Z/` used fresh checked-in-default vectors in `runs/eos-vector-caches/eos-nf005-current-default-scifact-child-w128-o32-128d/` and measured `5,183` parents, `12,468` children, and `128d` vectors through local flat `PutMultiVector`/exact `SearchParentsVector` with `packed_parent_multivectors`, `packed_metadata_mode=none`, and `packed_child_id_mode=ordinal`: q4 release-gate evidence recorded nDCG@10 `0.451122`, recall@100 `0.755222`, DB directory multiple `0.020372x`, vector payload multiple `0.013312x`, p95 `142.971679ms`, planner fit `180`, `target_child_fit=true`, and target storage multiple `0.554545x`; q8 diagnostic evidence recorded nDCG@10 `0.472867`, recall@100 `0.780222`, DB directory multiple `0.032901x`, p95 `22.988553ms`, planner fit `93`, and `target_child_fit=false`. The predecessor targeted-v3 packed-parent local flat run remains historical comparison/provenance only. Treat this as the local flat CorkScrewDB API counterpart to the cache-only budget-quality smoke and pure planner smoke, separate from q4/fp16 sidecar rerank evidence; it is not remote mode, federation, HNSW, q4/fp16 alias promotion, or native Matryoshka evidence. The q4 p95 is recorded as run evidence, not as a service SLO.
...
For the predecessor EOS-named targeted-v3 release package, keep dense promotion evidence separate from the current quantized deployment readiness. Its q4/fp16 overfetch250 compact gate required an explicit `recall@100` tolerance `0.00025` and is now predecessor evidence only. The current nf005 compact policy is q4/fp16 overfetch200 with total compression `1.5900621118x`; it remains the selected default profile because q4/175 failed direct checked-in FiQA recall and q4/200 is the lowest historical recovery-sweep profile that clears predecessor q4/fp16/250 quality across SciFact, NFCorpus, and FiQA by explicit comparison. Formal future compact gates use the seeded provenance-safe anchor, while compact-aware candidates must still disclose whether they clear the historical strict FiQA reference. Do not generalize this to direct q4, remote CorkScrewDB, HNSW, federation, or hosted-model parity.
...
Current dense promotion candidate: `assets/corkscrewdb-default-embedder/corkscrewdb-default-embedder.mll`, SHA256 `8074d2fce1842e232df2b4172d40463d82b57848c719b2d76fdd68aca682ac70`. It is the durable CorkScrewDB external-provider asset for the nf005 Qwen3-continuation release; the ignored run path `runs/current-release-qwen3-nf005-continuation-20260616T224102Z/candidate/eos-embed-v1.sealed.mll` remains provenance. It was trained from `qwen3.filtered.hard-negatives.jsonl` with teacher source weights `scifact=1,nfcorpus=0.05,fiqa=0.25`, LR `0.000002`, and quality target `pairwise`. The release package SHA256 is `188265db16992ab24be15e678c5f7e175bebad769e8d844e8b0f50ffc23bd5bf`; tokenizer SHA256 is `64cf63223cb3f97125040677a573e6ab6c625cff1f6f338f4e680a4c9f7a42f5`. Package and sealed inspection report `package verify: OK`, sealed inspection reports `package: embedded sealed MLL`, and final plus hard eval logs record `optimizer_updates=0`.
...
The predecessor targeted-v3 package at `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/` remains comparison provenance. The legacy source artifact `runs/eos-embed-v1-targeted-neargate-v3-low-lr-restorebest-20260614T000000Z/targeted-v3-lr000002-restorebest-manta/manta-embed-v1.sealed.mll`, SHA256 `ea776e2fca7fdade7ee05396b2ee8980e220899e2515853c83a4bca34cf87242`, remains provenance only. The previous strict sealed anchor is `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z/manta-embed-v1.sealed.mll`, SHA256 `a7461b47784ea7434cf6048f33f6c281ef19887cfa9d0c699b6f2fba079f2b67`, with sealed scoreboard under `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z-sealed-scoreboard/`. Its sealed-vs-train-package comparison recorded zero nonzero quality or count deltas against `runs/manta-embed-v1-deephard-full-ft-20260610T0000Z-scoreboard/`. Treat nf005 as a measured dense short-set promotion, not a broad release claim. Current release-hardening evidence also includes the nf005 CorkScrewDB local flat packed-parent API smoke in `runs/eos-nf005-current-default-corkscrewdb-budget-quality-packed-q4q8-20260617T142823Z/`: q4 records nDCG@10 `0.451122`, recall@100 `0.755222`, DB directory multiple `0.020372x`, vector payload multiple `0.013312x`, p95 `142.971679ms`, planner fit `180`, and target fit `true`; q8 is diagnostic because planner fit `93` leaves `target_child_fit=false`.
