...
2026-06-20 command-level evidence validated a newer candidate policy: `fiqa24-nf48` at `runs/eos-s40-longembed-balanced-anchor-sweep-v1-20260620T195017Z/candidates/fiqa24-nf48/candidate/eos-embed-v1.sealed.mll` with `eval-retrieval-hybrid --method minmax_blend --alpha 0.5 --top-k 100` passed SciFact `0.717644867485`/`0.932888888889`, NFCorpus `0.311158654714`/`0.290278895553`, and FiQA `0.219415915378`/`0.500980325402` nDCG@10/recall@100. Treat this as validated `eos-hybrid` ranking-policy evidence only: s40 remains the dense default, no shipped asset changes from this run, and NFCorpus has a command/offline nDCG@10 difference of `-0.002670461567` with the gate still passing. Next product-promotion step is to decide whether CorkScrewDB should expose or select this hybrid policy by default; if yes, run serving/API smoke for the selected policy before changing any default alias or asset.
