...
For the current targeted-v3 dense candidate, keep dense promotion separate from quantized deployment readiness. The q4/fp16 overfetch250 and q8/fp16 overfetch125 compact profiles have been refreshed against targeted-v3 and still fail strict zero-regression by the known NFCorpus fp16 boundary miss: exactly two dense rank-100 relevant docs, `PLAIN-1288` / `MED-2448` and `PLAIN-2680` / `MED-4195`, become fp16 rerank rank 101. q4/fp16 keeps total compression `1.590062x`; q8/fp16 keeps `1.326425x`; both preserve nDCG@10 across the three short-set datasets but miss NFCorpus recall@100 by `-0.000236936880015165`. The accepted compact policy is explicit: q4/fp16 overfetch250 is the compact RC only with `recall@100` tolerance `0.00025`; q8/fp16 overfetch125 is the lower-compression fallback because it has the same miss shape. Direct q8 diagnostics are non-regressing in the measured direct metrics, but direct rows are not the default-promotion rows. The q4 dense-sidecar control fixes NFCorpus but fails FiQA recall by `-0.0005144032921810537` and compresses only `0.885813x`, so it is not deployable compact evidence. Do not update compact aliases in wording or manifests as strict zero-regression promotions.
