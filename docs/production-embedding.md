...
The current dense-quality reranker-teacher frontier is request-only, not training input. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` produced `1,188` NFCorpus train/dev hard-negative examples and `7,066` scorer request rows. The working scorer path now exists: `cmd/teacher-bridge --mode tei-rerank` can score TEI `/rerank`, and RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9`. The decision remains `no_train` / `teacher_quality_blocked` because cleaned subset64 BGE audits are still quality-negative: `BAAI/bge-reranker-base` selected/any-positive top1 `29/64` (`0.453125`), kept `29/64`; `BAAI/bge-reranker-large` `22/64` (`0.343750`), kept `22/64`; and `BAAI/bge-reranker-v2-m3` `10/64` (`0.156250`), kept `10/64`. Cleaning removed the duplicate-positive-negative failure mode for these BGE audits (`duplicate_positive_negative_candidates=0`), with dirty-to-cleaned selected-top1 changes of base `+2`, large `+7`, and v2-m3 `+1`. Use `audit-teacher-scores --qrels <qrels.tsv> --corpus <corpus.jsonl>` when provenance exists to get duplicate/multi-positive-aware diagnostics in addition to the selected-positive defaults. Qwen3 seq-cls remains historical dirty-subset no-train evidence, not a cleaned rerun: TEI `120-1.9` failed with `classifier model type is not supported for Qwen3`, while Python `AutoModelForSequenceClassification` with Qwen3 model-card formatting on CUDA RTX 5070 Ti was mechanically scoreable; filtering kept `5/64`, cleared `59`, dropped `0`, and preserved request-only metadata with positive-top1 `0.078125` and mean margin `-0.5452850256115198`. Do not continue dense training from this artifact or substitute embedding/cosine/vector-cache teacher scores for the requested reranker lane. The next production action is to investigate label/teacher disagreement or test a different query-document teacher, then require positive-top1, margin, novelty, and leakage gates to pass before any full scoring/training descriptor.
