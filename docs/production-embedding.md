...
Default CorkScrewDB embedder promotion requires a vector-index cost check in addition to dense retrieval quality. The repo-local serving proxy is `scripts/smoke_eos_default_embedder_serving.fw`; for the promoted s40 compact policy, it selects the q4/fp16/rerank-overfetch=200 profile and records p50/p95/p99/max per-query scoring latency:
...
The release smoke verifies the checked-in artifact SHA256 `f494915a0d78b24205d5018bb701bf40cabbedee4bc8b96b6a1920b19131da5a`, tokenizer SHA256 `64cf63223cb3f97125040677a573e6ab6c625cff1f6f338f4e680a4c9f7a42f5`, package metadata, local `embed-text` load as `f16[256]`, and, when CorkScrewDB is supplied, the temp DB manifest embedding id `corkscrewdb-default-embedder` dim `256`.
...
Current-default command-level evidence validates the durable asset `assets/corkscrewdb-default-embedder/corkscrewdb-default-embedder.mll`, SHA256 `f494915a0d78b24205d5018bb701bf40cabbedee4bc8b96b6a1920b19131da5a`, with `eval-retrieval-hybrid --method minmax_blend --alpha 0.5 --top-k 100` in `runs/eos-s40-current-default-hybrid-policy-v1-20260621T103500Z/`: SciFact `0.717644867485`/`0.932888888889`, NFCorpus `0.311170830256`/`0.290149896585`, and FiQA `0.219415915378`/`0.500671683426` nDCG@10/recall@100, all positive versus the s40 dense anchor. The same run includes full uncapped CorkScrewDB BM25-dot `SearchMulti` smokes with `overall_pass=true`: SciFact `0.724378172602`/`0.932888888889`, NFCorpus `0.315090347196`/`0.293990495284`, and FiQA `0.223742712175`/`0.504002444743`. Treat this as validated current-default `eos-hybrid` ranking-policy evidence only: s40 remains the dense default, no shipped asset changes from this run, and the evidence covers local lexical+dense policy, not dense model promotion. Next product-promotion step is to decide whether CorkScrewDB should expose or select this hybrid policy by default; do not change any default alias or asset for this policy without a separate product decision.
