...
The same scaled q4 shape now has packed parent-object evidence. Run `EOS_CORKSCREW_SMOKE_LAYOUT=packed_parent_multivectors` with flat `quantized_only` storage to write one parent via `PutMultiVector` and query exact local flat parents with `SearchParentsVector`. Verified run `runs/eos-corkscrewdb-timeseries-window-scale-q4-100-variants20-packed-20260616T000000Z/` recorded `parent_insert_count=100`, `parent_search_exact=true`, `overfetch_children=0`, the same `0.683594x` vector payload multiple, DB directory multiple `2.005190x`, DB bytes `2,463,978`, `ndcg@10=0.352927`, `recall@100=1.000000`, and p95 `9.528395ms`. Packed DB bytes were `0.874198x` of the separate-child run, about `12.58%` lower, with p95 about `0.797467x` and load time about `0.191984x`. The recall improvement is an exact parent-rollup result versus the separate-child run's bounded overfetch, not a trained encoder quality gain.
