...
The current dense-quality reranker-teacher frontier is request-only, not training input. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` produced `1,188` NFCorpus train/dev hard-negative examples and `7,066` scorer request rows, but the decision is `no_train` / `scorer_blocked` because no local stronger cross-encoder or reranker scorer was found. Do not continue dense training from this artifact or substitute embedding/cosine/vector-cache teacher scores for the requested reranker lane. The next production action is to wire or acquire a true query-document joint scorer, score `nfcorpus-train-dev-novel-frontier.teacher-score-requests.jsonl`, import scores, and audit positive-top1, margins, novelty, and leakage before any training descriptor.
