...
Default CorkScrewDB embedder promotion requires a vector-index cost check in addition to dense retrieval quality. The repo-local serving proxy is `scripts/smoke_eos_default_embedder_serving.fw`; for the accepted targeted-v3 compact policy, it compares the q4/fp16/overfetch250 compact RC profile against the q8/fp16/overfetch125 fallback and records p50/p95/p99/max per-query scoring latency:
...
This smoke uses the in-repo TurboQuant evaluator as CorkScrewDB-relevant vector-index and serving proxy evidence. It is not an actual CorkScrewDB API load/index/search smoke. For the EOS-named targeted-v3 release package, q4/fp16 overfetch250 fails strict zero-regression only on NFCorpus recall@100 by `-0.000236936880015165`; under the explicit `recall@100` tolerance `0.00025`, it passes all selected compact checks and is the compact release-candidate profile. Full uncapped SciFact serving proxy run `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/serving-smoke-scifact-full/` measured q4/fp16/250 at nDCG@10 `0.5623217863`, recall@100 `0.7964444444`, total compression `1.5900621118x`, p95 `7.607784 ms`, `414712.08` scores/s, and `75,000` rerank scores; q8/fp16/125 is a lower-compression measured serving fallback, not the compact RC or compact-gate claim, with total compression `1.3264248705x`, p95 `15.364934 ms`, and `204234.21` scores/s. Both rows are tied to release sealed SHA256 `601932251333d6bb5279cbe6c042a7b7f188037a47501f3134071e51baa5f2f7`. For the local flat CorkScrewDB API path, run `scripts/smoke_corkscrewdb_child_vectors.fw`; by default it creates or consumes child-vector caches, loads child vectors with CorkScrewDB `PutVector`, queries with `SearchVector`, rolls child hits up to parent IDs by max score, and records storage, latency, nDCG@10, and recall@100:
...
For the EOS-named targeted-v3 release package, keep dense promotion separate from quantized deployment readiness. The release-artifact compact gate covers q4/fp16 overfetch250 only under the explicit `recall@100` tolerance `0.00025`; it still fails strict zero-regression by the known NFCorpus fp16 boundary miss: exactly two dense rank-100 relevant docs, `PLAIN-1288` / `MED-2448` and `PLAIN-2680` / `MED-4195`, become fp16 rerank rank 101. q4/fp16 keeps total compression `1.590062x` and is the compact RC only with that tolerance. q8/fp16 overfetch125 keeps `1.326425x` and has the same miss shape, so it is a lower-compression measured serving fallback, not the compact RC or compact-gate claim. Direct q8 diagnostics are non-regressing in the measured direct metrics, but direct rows are not the default-promotion rows. The q4 dense-sidecar control fixes NFCorpus but fails FiQA recall by `-0.0005144032921810537` and compresses only `0.885813x`, so it is not deployable compact evidence. Do not update compact aliases in wording or manifests as strict zero-regression promotions.
...
Current dense promotion candidate: `assets/corkscrewdb-default-embedder/corkscrewdb-default-embedder.mll`, SHA256 `601932251333d6bb5279cbe6c042a7b7f188037a47501f3134071e51baa5f2f7`. It is the durable CorkScrewDB external-provider asset for the EOS-named targeted-v3 release package; the ignored run path `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/eos-embed-v1.sealed.mll` remains provenance. It was trained from `43` extremely targeted rows with no `teacher_scores`, and its release scoreboard `runs/eos-embed-v1-targeted-v3-release-package-20260616T000000Z/scoreboard/scoreboard.json` exactly matches the targeted-v3 dense short-set rows. The release package SHA256 is `188265db16992ab24be15e678c5f7e175bebad769e8d844e8b0f50ffc23bd5bf`; tokenizer SHA256 is `64cf63223cb3f97125040677a573e6ab6c625cff1f6f338f4e680a4c9f7a42f5`. Package and sealed inspection report `package verify: OK`, sealed inspection reports `package: embedded sealed MLL`, and final plus hard eval logs record `optimizer_updates=0`.
