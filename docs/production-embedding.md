...
The current dense-quality reranker-teacher frontier is request-only, not training input. `runs/eos-narrow-default-reranker-teacher-frontier-audit-v1-20260623T065159Z/` produced the older NFCorpus train/dev hard-negative frontier and proved the scorer path: `cmd/teacher-bridge --mode tei-rerank` can score TEI `/rerank`, and RTX50/Blackwell hosts should use `ghcr.io/huggingface/text-embeddings-inference:120-1.9`. The broader selected-diverse BGE audit at `runs/eos-nfcorpus-broader-cleaned-mining-quality-signal-v1-20260623T101157Z/` remains audit-only: `BAAI/bge-reranker-base` selected/any-positive top1 `59/128` (`0.460938`), `BAAI/bge-reranker-large` `65/128` (`0.507812`), and `BAAI/bge-reranker-v2-m3` `59/128` (`0.460938`), all with duplicate-positive negative candidates `0`. BGE large is the strongest BGE signal on that slice, but agreement and margins remain too weak for training. The MiniLM control did not change the decision: `cross-encoder/ms-marco-MiniLM-L6-v2` selected/any-positive top1 `54/128` (`0.421875`) with selected/any mean margin `0.140112`, weaker than BGE-large and heavily overlapping it.

The cleaned full-frontier Jina audit is the current production decision record. `runs/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-20260623T113125Z/` rebuilt the NFCorpus frontier with doc-id provenance, selected `1188` examples (`932` train + `256` dev), exported/scored `7128/7128` request rows, imported `1188/1188` examples with missing `0`, and passed validation with duplicate qrels/text positive negatives `0`, duplicate `(source, query, candidate)` request keys `0`, and selected unscored `teacher_scores` rows `0`. Jina selected/any-positive top1 was `160/1188 = 0.134680`, mean margin `-1.182826`, and mean normalized entropy `0.883789`; the filter kept `160`, cleared `1028`, and dropped `0`. Train split top1 was `122/932 = 0.130901`; dev split was `38/256 = 0.148438`. Decision: `teacher_quality_blocked` / `no_train`; do not continue dense training from these rows or propose a guarded trainability descriptor. The strategic conclusion changed from "old full frontier was contaminated" to "after a clean provenance-safe rebuild, Jina still fails the teacher-quality gate." Preserve `.tiller/scratch/codex/eos-nfcorpus-cleaned-full-reranker-frontier-jina-audit-v1-report.md` as the compact report. The low-space incident was handled by removing only this run's Hugging Face cache; scored artifacts, logs, and manifests are preserved. Next production action should pivot away from Jina toward BGE-large/mxbai compatibility, synthetic or non-overlapping FIQA/SciFact-compatible signal, changed compact/Matryoshka objectives, or larger-model/bootstrap. Do not train from the old dirty full-frontier artifacts either.
