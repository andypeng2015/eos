...
func printUsage() {
	...
	fmt.Println("  eos import-pretrained-bert --source <hf-snapshot-dir> [--model-name name] [--plan-json plan.json] [--verify-weights]")
	...
	fmt.Println("import-pretrained-bert writes a plan-only BERT-family HF config/tensor map and optional WordPiece tokenizer smoke check; --verify-weights performs metadata-only safetensors header validation and does not import weights or build a BERT graph.")
