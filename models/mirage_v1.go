...
// MirageV1Config controls the built-in Mirage Image v1 Eos artifact shape.
...
func DefaultMirageV1Module(cfg MirageV1Config) (*eosartifact.Module, error) {
	cfg = cfg.normalized()
	...
	mod := eosartifact.NewModule(cfg.Name)
	...
	addAnalysis := func(entry string) {
		mod.Steps = append(mod.Steps,
			step(entry, eosartifact.StepConv2D, "analysis_0", []string{"x", "ga0_weight", "ga0_bias"}, []string{"ga0"}, convAttrs("2", "2", "1")),
			step(entry, eosartifact.StepGDN, "analysis_gdn_0", []string{"ga0", "gdn0_beta", "gdn0_gamma"}, []string{"ga0_gdn"}, nil),
			step(entry, eosartifact.StepConv2D, "analysis_1", []string{"ga0_gdn", "ga1_weight", "ga1_bias"}, []string{"ga1"}, convAttrs("2", "2", "1")),
			step(entry, eosartifact.StepGDN, "analysis_gdn_1", []string{"ga1", "gdn1_beta", "gdn1_gamma"}, []string{"ga1_gdn"}, nil),
			step(entry, eosartifact.StepConv2D, "analysis_2", []string{"ga1_gdn", "ga2_weight", "ga2_bias"}, []string{"ga2"}, convAttrs("2", "2", "1")),
			step(entry, eosartifact.StepGDN, "analysis_gdn_2", []string{"ga2", "gdn2_beta", "gdn2_gamma"}, []string{"ga2_gdn"}, nil),
			step(entry, eosartifact.StepConv2D, "analysis_3", []string{"ga2_gdn", "ga3_weight", "ga3_bias"}, []string{"y"}, convAttrs("2", "2", "1")),
			step(entry, eosartifact.StepTurboQEncode, "latent_quantize", []string{"y"}, []string{"c_coords", "c_norms"}, commonAttrs),
		)
	}
	addHyperAnalysis := func(entry string) {
		mod.Steps = append(mod.Steps,
			step(entry, eosartifact.StepTurboQDecode, "latent_dequantize", []string{"c_coords", "c_norms"}, []string{"y_hat"}, commonAttrs),
			step(entry, eosartifact.StepConv2D, "hyper_analysis", []string{"y_hat", "ha_weight", "ha_bias"}, []string{"z"}, convAttrs("1", "1", "1")),
			step(entry, eosartifact.StepTurboQEncode, "hyper_quantize", []string{"z"}, []string{"c_z", "c_z_norms"}, commonAttrs),
			step(entry, eosartifact.StepTurboQDecode, "hyper_dequantize", []string{"c_z", "c_z_norms"}, []string{"z_hat"}, commonAttrs),
			step(entry, eosartifact.StepConv2D, "hyper_synthesis_logits", []string{"z_hat", "hs_logits_weight", "hs_logits_bias"}, []string{"pi_logits"}, convAttrs("1", "1", "1")),
			step(entry, eosartifact.StepConv2D, "hyper_synthesis_norms", []string{"z_hat", "hs_norm_weight", "hs_norm_bias"}, []string{"norm_params"}, convAttrs("1", "1", "1")),
		)
	}
	...
	addSynthesisLayers := func(entry, latentInput string) {
		mod.Steps = append(mod.Steps,
			step(entry, eosartifact.StepConv2DTrans, "synthesis_0", []string{latentInput, "gs0_weight", "gs0_bias"}, []string{"gs0"}, convTransposeAttrs("2", "2", "1", "1")),
			step(entry, eosartifact.StepIGDN, "synthesis_igdn_0", []string{"gs0", "igdn0_beta", "igdn0_gamma"}, []string{"gs0_igdn"}, nil),
			step(entry, eosartifact.StepConv2DTrans, "synthesis_1", []string{"gs0_igdn", "gs1_weight", "gs1_bias"}, []string{"gs1"}, convTransposeAttrs("2", "2", "1", "1")),
			step(entry, eosartifact.StepIGDN, "synthesis_igdn_1", []string{"gs1", "igdn1_beta", "igdn1_gamma"}, []string{"gs1_igdn"}, nil),
			step(entry, eosartifact.StepConv2DTrans, "synthesis_2", []string{"gs1_igdn", "gs2_weight", "gs2_bias"}, []string{"gs2"}, convTransposeAttrs("2", "2", "1", "1")),
			step(entry, eosartifact.StepIGDN, "synthesis_igdn_2", []string{"gs2", "igdn2_beta", "igdn2_gamma"}, []string{"gs2_igdn"}, nil),
			step(entry, eosartifact.StepConv2DTrans, "synthesis_3", []string{"gs2_igdn", "gs3_weight", "gs3_bias"}, []string{"x_hat"}, convTransposeAttrs("2", "2", "1", "1")),
		)
	}
	...
	mod.Steps = append(mod.Steps,
		step("train_step", eosartifact.StepCrossEntropy, "rate_hyperprior", []string{"c_z", "prior_z_logits"}, []string{"rate_z"}, zRateAttrs),
		step("train_step", eosartifact.StepCrossEntropy, "rate_coords", []string{"c_coords", "pi_logits"}, []string{"rate_coords"}, coordRateAttrs),
		step("train_step", eosartifact.StepCrossEntropy, "rate_norms", []string{"c_norms", "norm_params"}, []string{"rate_norms"}, normRateAttrs),
		step("train_step", eosartifact.StepScalarAdd, "rate_sum", []string{"rate_z", "rate_coords", "rate_norms"}, []string{"rate"}, nil),
	)
	addSynthesisLayers("train_step", "y")
	mod.Steps = append(mod.Steps,
		step("train_step", eosartifact.StepMSELoss, "distortion_mse", []string{"x", "x_hat"}, []string{"mse"}, nil),
		step("train_step", eosartifact.StepMSSSIMLoss, "distortion_ms_ssim", []string{"x", "x_hat"}, []string{"ms_ssim"}, nil),
		step("train_step", eosartifact.StepRDLoss, "loss", []string{"mse", "rate"}, []string{"loss"}, map[string]string{
			"lambda":     fmt.Sprintf("%.8g", cfg.Lambda),
			"rate_scale": fmt.Sprintf("%.8g", rateScale),
		}),
		step("train_step", eosartifact.StepReturn, "return", nil, []string{"loss", "x_hat", "c_z", "c_z_norms", "c_coords", "c_norms", "pi_logits", "norm_params", "rate", "rate_z", "rate_coords", "rate_norms", "mse", "ms_ssim"}, nil),
	)
	addAnalysis("analyze")
	addHyperAnalysis("analyze")
	mod.Steps = append(mod.Steps, step("analyze", eosartifact.StepReturn, "return", nil, []string{"c_z", "c_z_norms", "c_coords", "c_norms", "pi_logits", "norm_params"}, nil))
	addHyperSynthesis("synthesize_hyperprior")
	mod.Steps = append(mod.Steps, step("synthesize_hyperprior", eosartifact.StepReturn, "return", nil, []string{"pi_logits", "norm_params"}, nil))
	addSynthesis("synthesize_image")
	mod.Steps = append(mod.Steps, step("synthesize_image", eosartifact.StepReturn, "return", nil, []string{"x_hat"}, nil))
...
func InitMirageV1Artifact(path string, cfg MirageV1Config) error {
	...
	return eosartifact.WriteFile(path, mod)
}
...
func paramTensor(name, dtype string, shape []string) eosartifact.Param {
...
func valueBinding(name, dtype string, shape []string) eosartifact.ValueBinding {
	return eosartifact.ValueBinding{Name: name, Type: tensorType(dtype, shape)}
}

func tensorType(dtype string, shape []string) eosartifact.ValueType {
...
func buffer(name, dtype string, shape []string) eosartifact.Buffer {
	return eosartifact.Buffer{Name: name, DType: dtype, Shape: append([]string(nil), shape...)}
}

func step(entry string, kind eosartifact.StepKind, name string, inputs, outputs []string, attrs map[string]string) eosartifact.Step {
